...
class DeduplicationService:
    ...
    def annotate(self, mentions: list[MentionRecord]) -> tuple[list[MentionRecord], DedupAnalysisResult]:
        if not mentions:
        ...
        updated_mentions = [
            mention.model_copy(update=annotations.get(mention.mention_id, {}))
        ...
        return (
            ...
            DedupAnalysisResult(
                ...
                exact_cluster_count=sum(1 for cluster in exact_clusters if cluster.cluster_size > 1),
                near_cluster_count=sum(1 for cluster in near_clusters if cluster.cluster_size > 1),
                clusters=[*exact_clusters, *near_clusters],
            ),
        )

    def _annotation_map(self, clusters: list[DedupClusterRecord]) -> dict[str, dict[str, object]]:
    ...
    def _exact_clusters(self, mentions: list[MentionRecord]) -> list[DedupClusterRecord]:
        ...
        for fingerprint, members in sorted(groups.items()):
            ...
            clusters.append(
                DedupClusterRecord(
                    ...
                    mention_ids=[member.mention_id for member in members],
    ...
    def _near_clusters(
        ...
        *,
        ...
        candidates = [
            mention
            for mention in mentions
            if mention.mention_id not in exact_members
            and len(mention.normalized_text_compact) >= self.min_text_length
        ]
        if len(candidates) < 2:
            return []
        shingles = {
            mention.mention_id: self._shingles(mention.normalized_text_compact)
            for mention in candidates
        }
        signatures = {
            mention_id: self._minhash_signature(items)
            for mention_id, items in shingles.items()
            if items
        }
        if len(signatures) < 2:
            return []
        ...
        for group_members in grouped.values():
            ...
            clusters.append(
                DedupClusterRecord(
                    ...
                    representative_mention_id=representative.mention_id,
                    representative_text=representative.raw_text,
                    mention_ids=sorted(member.mention_id for member in group_members),
    ...
    def _shingles(self, text: str) -> set[str]:
        ...
        if len(tokens) >= self.word_shingle_size:
            return {
                " ".join(tokens[index : index + self.word_shingle_size])
                for index in range(len(tokens) - self.word_shingle_size + 1)
            }
        compact = text.replace(" ", "")
        if len(compact) >= self.char_shingle_size:
            return {
                compact[index : index + self.char_shingle_size]
                for index in range(len(compact) - self.char_shingle_size + 1)
            }
    ...
    def _minhash_signature(self, shingles: set[str]) -> tuple[int, ...]:
        ...
        for seed in range(self.num_perm):
            signature.append(min((value ^ ((seed + 1) * 0x9E3779B97F4A7C15)) & 0xFFFFFFFFFFFFFFFF for value in hashed))
    ...
    def _representative(self, mentions: list[MentionRecord]) -> MentionRecord:
        return sorted(
            mentions,
            key=lambda mention: (
                -len(mention.normalized_text_compact),
                mention.depth,
                mention.mention_id,
            ),
        )[0]
    ...
    def _cluster_similarity(
        ...
        for left_index in range(len(mentions)):
            for right_index in range(left_index + 1, len(mentions)):
                total += self._jaccard(
                    shingles[mentions[left_index].mention_id],
                    shingles[mentions[right_index].mention_id],
                )
                pair_count += 1
        if pair_count <= 0:
            return 1.0
