...
def apply_topic_lineage(
    artifact: TopicArtifactFact,
    ...
    if match is None:
        return artifact.model_copy(
            update={
                "effective_topic_key": artifact.effective_topic_key or artifact.topic_key,
                "effective_topic_label": artifact.effective_topic_label or artifact.topic_label,
                "reviewed_topic_id": artifact.reviewed_topic_id or artifact.topic_key,
                "topic_lineage_id": artifact.topic_lineage_id or default_topic_lineage_id(artifact),
                "topic_review_resolution": resolution,
            }
        )
    ...
    return artifact.model_copy(
        update={
            ...
            "reviewed_topic_id": match.reviewed_topic_id,
            "topic_lineage_id": match.topic_lineage_id,
            "reviewed_merge_target_key": match.merge_into_topic_key,
            "reviewed_label_override": match.canonical_topic_label,
            "reviewed_usefulness_judgment": match.usefulness_judgment,
            "reviewed_stability_judgment": match.stability_judgment,
            "topic_review_resolution": resolution,
        }
    )


def default_topic_lineage_id(artifact: TopicArtifactFact) -> str:
    base = (
        artifact.effective_topic_key
        or artifact.topic_profile_signature
        or artifact.topic_signature
        or artifact.topic_term_signature
        or artifact.topic_key
    )
    return normalize_alias(base) or base


def topic_lineage_matches(current: TopicArtifactFact, previous: TopicArtifactFact) -> bool:
...
def resolve_topic_lineage(
    artifact: TopicArtifactFact,
    records: list[PromotedTopicLineageRecord],
) -> tuple[PromotedTopicLineageRecord | None, str]:
    matches = _matching_records(
        records,
        topic_profile_signature=artifact.topic_profile_signature,
        topic_signature=artifact.topic_signature,
        topic_term_signature=artifact.topic_term_signature,
    ...
    resolved_identities = {
        (
            record.canonical_topic_key,
            record.reviewed_topic_id,
            record.topic_lineage_id,
        )
    ...
    match_kind = {
        4: "topic_profile_signature",
        3: "topic_signature",
        2: "topic_term_signature",
        1: "topic_key",
    }.get(highest_priority, "raw")
...
def _matching_records(
    ...
    *,
    ...
    topic_key: str,
) -> list[tuple[int, PromotedTopicLineageRecord]]:
    if not records:
        return []
    matches: list[tuple[int, PromotedTopicLineageRecord]] = []
