...
class AliasRegistry:
        ...
        *,
        ...
        for entity_seed in registry.entities:
            entity = CanonicalEntity(
                ...
                entity_kind=entity_seed.entity_kind,
                knowledge_layer=entity_seed.knowledge_layer,
                active_linking=entity_seed.active_linking,
                target_eligible=entity_seed.target_eligible,
                taxonomy_ids=entity_seed.taxonomy_ids,
                description=entity_seed.description,
                anti_confusion_phrases=entity_seed.anti_confusion_phrases,
            )
            ...
            for alias_value in [entity_seed.name, *entity_seed.aliases, *entity_seed.compact_aliases]:
                cls._register_alias(
                    aliases_by_exact=aliases_by_exact,
                    aliases_by_normalized=aliases_by_normalized,
                    alias=EntityAlias(
                        alias_id=f"{entity.canonical_entity_id}:{normalize_alias(alias_value)}",
        ...
        for contribution in registry.alias_contributions:
            ...
            cls._register_alias(
                ...
                alias=EntityAlias(
                    alias_id=(
                        f"{contribution.canonical_entity_id}:"
                        f"{normalize_alias(contribution.alias)}:{contribution.source}"
                    ),
                    canonical_entity_id=contribution.canonical_entity_id,
                    alias=contribution.alias,
                    normalized_alias=normalize_alias(contribution.alias),
        ...
        return cls(
            ...
            aliases_by_normalized=aliases_by_normalized,
            noise_terms={
                normalize_alias(item.term)
                for item in registry.noise_terms
                if applicability_engine.noise_term_applies(item, review_context)
            },
        )
    ...
    def candidate_strings(self) -> dict[str, str]:
        return {entity_id: entity.name for entity_id, entity in self.entities.items()}

    def vector_alias_strings(self) -> dict[str, tuple[str, str, str]]:
        return {
            alias.alias_id: (alias.canonical_entity_id, alias.alias, alias.source)
            for alias in self.all_aliases()
        }

    def all_aliases(self) -> Iterable[EntityAlias]:
        seen: set[str] = set()
    ...
    def find_exact_aliases(self, text: str) -> list[EntityAlias]:
        return list(self.aliases_by_exact.get(text.casefold(), []))

    def find_normalized_aliases(self, text: str) -> list[EntityAlias]:
        return list(self.aliases_by_normalized.get(normalize_alias(text), []))

    def is_noise_term(self, text: str) -> bool:
...
def boundary_contains(text: str, needle: str) -> bool:
    if not text or not needle:
        return False
    pattern = re.compile(rf"(?<!\w){re.escape(needle)}(?!\w)", flags=re.IGNORECASE | re.UNICODE)
