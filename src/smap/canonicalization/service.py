...
from smap.canonicalization.models import (
    CanonicalEntity,
    EntityAlias,
    EntityCandidate,
    ResolutionDecision,
...
def _significant_tokens(text: str) -> set[str]:
    return {
        token
        for token in normalize_alias(text).split()
...
class CanonicalizationEngine:
    def __init__(
        ...
        fuzzy_threshold: float = 91.0,
        embedding_threshold: float = 0.75,
        vector_namespace: str = "canonical_entities",
        embedding_rerank_enabled: bool = True,
        ...
        self.verification_level: VerifierLevel = verification_level
        ...
        self._entity_strings = (
            {
                entity_id: bundle.alias_lookup_text
                for entity_id, bundle in self.prototype_registry.entities.items()
            }
            if self.prototype_registry is not None
            else self.alias_registry.candidate_strings()
        )
    ...
    def resolve(self, candidate: EntityCandidate) -> ResolutionDecision:
        ...
        shortlist = self._candidate_shortlist_ids(
            candidate,
        ...
        ranked_matches = self._rank_embedding_candidates(
            candidate,
            normalized_candidate,
            shortlist=shortlist,
            signal_map=signal_map,
        ...
        if (
            candidate.entity_type_hint is not None
            and top_match.metadata.get("type_consistent") is False
            and top_match.score >= max(self.embedding_threshold - 0.05, 0.6)
        ):
            return self._unresolved(
                candidate,
                reason="type_conflict",
                candidate_canonical_ids=[match.candidate_id for match in ranked_matches],
            )
        if self._embedding_result_is_ambiguous(candidate, ranked_matches):
            return self._unresolved(
                candidate,
                reason="ambiguous_multi_candidate",
                candidate_canonical_ids=[match.candidate_id for match in ranked_matches],
            )
        ...
        if top_match.score < score_floor:
            unresolved_reason = (
                "embedding_surface_support_insufficient"
                if _metadata_float(top_match.metadata.get("alias_surface_support")) >= 0.38
        ...
        if not self._embedding_surface_support(
            candidate,
            ...
            return self._unresolved(
                candidate,
                reason="embedding_surface_support_insufficient",
                candidate_canonical_ids=[match.candidate_id for match in ranked_matches],
            )
        return self._resolve_ranked_match(
            candidate,
            ranked_matches,
            signal_map=signal_map,
        )
    ...
    def _collect_candidate_signals(
        ...
        for alias in self._prefilter_aliases(normalized_candidate):
            ...
            if len(normalized_candidate) >= 4 and abs(len(alias.normalized_alias) - len(normalized_candidate)) <= 12:
                fuzzy_score = max(
                    float(fuzz.ratio(normalized_candidate, alias.normalized_alias)) / 100.0,
                    float(fuzz.token_set_ratio(normalized_candidate, alias.normalized_alias)) / 100.0,
                )
                if fuzzy_score >= 0.91:
                    self._update_alias_signal(signal_map, alias, signal_kind="fuzzy_alias", score=min(fuzzy_score, 0.9))
        return signal_map

    def _update_alias_signal(
        self,
        signal_map: dict[str, _CandidateSignals],
        alias: EntityAlias,
        *,
    ...
    def _candidate_shortlist_ids(
        ...
        *,
        ...
        prototype_shortlist = self._shortlist_prototype_candidates(
            candidate,
            normalized_candidate,
            seed_candidate_ids=set(signal_map),
        )
    ...
    def _looks_like_unique_prefix_candidate(
        self,
        candidate: EntityCandidate,
        normalized_candidate: str,
        alias: EntityAlias,
    ...
    def _candidate_score_floor(
        self,
        candidate: EntityCandidate,
        signal: _CandidateSignals | None,
    ) -> float:
    ...
    def _resolve_ranked_match(
        self,
        candidate: EntityCandidate,
        ranked_matches: list[SimilarityMatch],
        *,
        signal_map: dict[str, _CandidateSignals],
    ) -> ResolutionDecision:
        ...
        return self._decision_for_entity(
            candidate,
            entity,
            matched_alias=matched_alias,
            matched_by=matched_by,
            confidence=round(top_match.score, 3),
            provider_version=f"{self.embedding_provider.version}+{provider_suffix}",
            candidate_canonical_ids=[match.candidate_id for match in ranked_matches],
        )

    def _embedding_query_text(self, candidate: EntityCandidate) -> str:
        parts = [
            candidate.text,
            candidate.context_text or "",
            candidate.surrounding_text or "",
        ]
        return " || ".join(part for part in parts if part)

    def _embedding_view_texts(
        self,
        candidate: EntityCandidate,
    ...
    def _embedding_surface_support(
        self,
        candidate: EntityCandidate,
        candidate_id: str,
        *,
        signal: _CandidateSignals | None = None,
    ) -> bool:
        normalized_candidate = candidate.normalized_text or normalize_alias(candidate.text)
        candidate_tokens = _significant_tokens(normalized_candidate)
    ...
    def _candidate_is_mention_worthy(
        self,
        candidate: EntityCandidate,
        normalized_candidate: str,
        *,
        signal_map: dict[str, _CandidateSignals] | None,
    ) -> bool:
        specificity = self._surface_specificity(candidate, normalized_candidate)
        signal_map = signal_map or {}
        if any(
            signal.exact_alias >= 0.99
            or signal.normalized_alias >= 0.92
            or signal.compact_alias >= 0.9
            or max(signal.boundary_support, signal.contextual_support, signal.unique_prefix_support) >= 0.62
        ...
        if self.prototype_registry is None:
            mention_score = self._mention_worthiness_score(
                candidate,
                normalized_candidate,
                signal_map=signal_map,
                specificity=specificity,
                prototype_top_score=prototype_top_score,
            )
            required = 0.48 if candidate.entity_type_hint in {"brand", "product", "product_line"} else 0.52
            if candidate.discovered_by and all(method in _LOW_PRECISION_DISCOVERY for method in candidate.discovered_by):
                required += 0.04
            return mention_score >= required
        shortlisted = self._shortlist_prototype_candidates(
            candidate,
            normalized_candidate,
            seed_candidate_ids=set(signal_map),
        )
        if not shortlisted:
            return False
        ranked = self._rank_prototype_candidates(
            candidate,
            normalized_candidate,
            shortlisted,
            signal_map=signal_map,
        ...
        threshold = 0.48 if candidate.entity_type_hint in {"brand", "product", "product_line"} else 0.52
        if self.verification_level == "c" and candidate.entity_type_hint == "concept":
            threshold += 0.04
        if candidate.discovered_by and all(
            method in _LOW_PRECISION_DISCOVERY for method in candidate.discovered_by
        ):
            threshold += 0.04
        if mention_score >= threshold:
            return True
        required_score = 0.54 if token_count <= 1 else 0.5
        if candidate.entity_type_hint is None and candidate.discovered_by and all(
            method in _LOW_PRECISION_DISCOVERY for method in candidate.discovered_by
        ):
            required_score += 0.06
        return bool(ranked and ranked[0].score >= required_score)

    def _mention_worthiness_score(
        self,
        candidate: EntityCandidate,
        normalized_candidate: str,
        *,
        signal_map: dict[str, _CandidateSignals] | None,
        specificity: float,
        prototype_top_score: float,
    ) -> float:
        features = self._mention_worthiness_features(
            candidate,
            normalized_candidate,
            signal_map=signal_map,
            specificity=specificity,
        ...
        score = (
            features["specificity"] * 0.26
            + features["strongest_alias_signal"] * 0.24
            + features["prototype_top_score"] * 0.22
            + features["candidate_confidence"] * 0.06
            + features["token_count"] * 0.06
            + features["has_digits"] * 0.04
            + features["is_high_signal_discovery"] * 0.06
            + features["distinctive_surface"] * 0.06
            + features["type_brand_like"] * 0.04
            + features["type_product_like"] * 0.05
            - features["type_concept"] * 0.04
            - features["is_low_precision_only"] * 0.08
        )
        if self.verification_level in {"b", "c"}:
            verifier_bonus = (
                min(features["prototype_top_score"], features["strongest_alias_signal"] or 1.0) * 0.08
                + max(features["prototype_top_score"] - 0.62, 0.0) * 0.12
            )
            if features["type_product_like"] > 0.0 and features["has_digits"] > 0.0:
                verifier_bonus += 0.06
            if features["type_brand_like"] > 0.0 and features["distinctive_surface"] > 0.0:
                verifier_bonus += 0.04
            score += verifier_bonus
        if self.verification_level == "c":
            if (
                features["strongest_alias_signal"] < 0.42
                and features["prototype_top_score"] < 0.68
                and features["specificity"] < 0.34
            ):
                score -= 0.08
            if features["type_concept"] > 0.0 and features["prototype_top_score"] < 0.74:
                score -= 0.05
        return round(max(min(score, 0.995), 0.0), 6)

    def _should_attempt_embedding(self, candidate: EntityCandidate, normalized_candidate: str) -> bool:
        specificity = self._surface_specificity(candidate, normalized_candidate)
        candidate_tokens = _significant_tokens(normalized_candidate)
        if specificity < 0.15 and len(candidate_tokens) == 1 and normalized_candidate in _GENERIC_SURFACE_TOKENS:
            return False
    ...
    def _shortlist_prototype_candidates(
        self,
        candidate: EntityCandidate,
        normalized_candidate: str,
        *,
        seed_candidate_ids: set[str] | None = None,
    ) -> list[str]:
        if self.prototype_registry is None:
            return []
        ...
        if candidate.entity_type_hint is not None:
            type_filtered = [
                entity_id
                for entity_id, bundle in self.prototype_registry.entities.items()
                if bundle.entity_type == candidate.entity_type_hint
            ]
    ...
    def _rank_embedding_candidates(
        ...
        *,
        ...
        signal_map: dict[str, _CandidateSignals] | None = None,
        ...
        if self.prototype_registry is None and signal_map:
            ...
            ranked = self._rank_signal_candidates(
                candidate,
                normalized_candidate,
                shortlist,
                signal_map=signal_map,
        ...
        if not shortlist:
            return []
        return self._rank_prototype_candidates(
            candidate,
            normalized_candidate,
            shortlist,
            signal_map=signal_map or {},
    ...
    def _rank_signal_candidates(
        ...
        *,
        ...
        for candidate_id in candidate_ids:
            ...
            compact_exact = max(
                1.0 if compact_candidate and compact_candidate in self._entity_compact_aliases(candidate_id) else 0.0,
                signal.compact_alias,
            )
            ...
            anti_confusion_penalty = (
                0.34
                if entity.anti_confusion_phrases
                and normalized_context
                and any(phrase in normalized_context for phrase in entity.anti_confusion_phrases[:6])
                and specificity < 0.32
                else 0.0
            )
            signal_confidence = max(
                0.0,
                0.94 + (signal.exact_alias * 0.04) if signal.exact_alias > 0.0 else 0.0,
                0.88 + (signal.normalized_alias * 0.08) if signal.normalized_alias > 0.0 else 0.0,
                0.84 + (compact_exact * 0.12) if compact_exact > 0.0 else 0.0,
                0.52 + (signal.boundary_support * 0.38) if signal.boundary_support > 0.0 else 0.0,
                0.5 + (signal.contextual_support * 0.34) if signal.contextual_support > 0.0 else 0.0,
                0.5 + (signal.unique_prefix_support * 0.3) if signal.unique_prefix_support > 0.0 else 0.0,
                0.42 + (signal.fuzzy_support * 0.24) if signal.fuzzy_support > 0.0 else 0.0,
            )
            score = (
                signal_confidence
                + alias_surface_support * 0.08
                + min(candidate.confidence, 1.0) * 0.04
                + source_bonus
                + signal.overlay_support
                + type_bonus
                - generic_penalty
                - concept_penalty
                - anti_confusion_penalty
            ...
            features = self._entity_link_features(
                alias_exact=signal.exact_alias,
                normalized_alias=signal.normalized_alias,
                compact_alias=compact_exact,
                boundary_support=signal.boundary_support,
                contextual_alias_support=signal.contextual_support,
                unique_prefix_support=signal.unique_prefix_support,
                fuzzy_support=signal.fuzzy_support,
                alias_surface_support=alias_surface_support,
                span_similarity=0.0,
                local_similarity=0.0,
                full_similarity=0.0,
                thread_similarity=0.0,
                candidate_confidence=min(candidate.confidence, 1.0),
                source_bonus=source_bonus,
                contextual_support=0.0,
                overlay_support=signal.overlay_support,
                layer_bonus=0.04 if entity.knowledge_layer != "base" else 0.0,
                entity_kind_bonus=0.03 if entity.entity_kind == "entity" and entity.target_eligible else 0.0,
                type_bonus=type_bonus,
                concept_penalty=concept_penalty,
                generic_penalty=generic_penalty,
                anti_confusion_penalty=anti_confusion_penalty,
                specificity=specificity,
                type_consistent=type_consistent,
                entity_kind=entity.entity_kind,
                entity_type=entity.entity_type,
                target_eligible=entity.target_eligible,
            )
            verifier_score = self._entity_verifier_score(features)
            final_score = (
                (verifier_score * 0.58) + (score * 0.42)
                if self.verification_level in {"b", "c"}
                else score
            )
            if self.verification_level == "c" and not type_consistent:
                final_score -= 0.05
            ranked.append(
                SimilarityMatch(
                    candidate_id=candidate_id,
                    score=round(max(min(final_score, 0.995), 0.0), 6),
                    candidate_text=entity.name,
                    metadata={
                        **features,
                        "alias_surface_support": round(alias_surface_support, 6),
                        "contextual_support": round(signal.contextual_support, 6),
                        "boundary_support": round(signal.boundary_support, 6),
                        "unique_prefix_support": round(signal.unique_prefix_support, 6),
                        "fuzzy_support": round(signal.fuzzy_support, 6),
                        "normalized_alias": round(signal.normalized_alias, 6),
                        "compact_alias": round(compact_exact, 6),
                        "type_consistent": type_consistent,
                        "candidate_entity_type": entity.entity_type,
                        "candidate_entity_kind": entity.entity_kind,
                        "anti_confusion_penalty": round(anti_confusion_penalty, 6),
    ...
    def _rank_prototype_candidates(
        self,
        candidate: EntityCandidate,
        normalized_candidate: str,
        candidate_ids: list[str],
        *,
        ...
        if self.prototype_registry is None:
            return []
        bundles = [
            self.prototype_registry.entities[candidate_id]
            for candidate_id in candidate_ids
            if candidate_id in self.prototype_registry.entities
            and self.prototype_registry.entities[candidate_id].vector is not None
        ]
        if not bundles:
            return []
        span_text, local_text, full_text, thread_text = self._embedding_view_texts(candidate)
        span_vector, local_vector, full_vector, thread_vector = self.embedding_provider.embed_texts(
            [span_text, local_text, full_text, thread_text],
            purpose=EmbeddingPurpose.LINKING,
        )
        compact_candidate = normalized_candidate.replace(" ", "")
        normalized_context = normalize_alias(" || ".join(part for part in (candidate.context_text, candidate.full_text) if part))
        specificity = self._surface_specificity(candidate, normalized_candidate)
        ...
        for bundle in bundles:
            ...
            anti_confusion_penalty = (
                0.32
                if bundle.anti_confusion_phrases
                and normalized_context
                and any(phrase in normalized_context for phrase in bundle.anti_confusion_phrases[:6])
                and specificity < 0.32
                else 0.0
            )
            ...
            concept_penalty = (
                0.18
                if bundle.entity_kind == "concept" and specificity >= 0.34 and not alias_exact and not compact_exact
                else 0.0
            )
            generic_penalty = 0.08 if specificity < 0.15 and bundle.entity_kind == "entity" else 0.0
            score = (
                alias_exact * 0.29
                + normalized_alias * 0.16
                + compact_exact * 0.22
                + boundary_support * 0.12
                + contextual_alias_support * 0.1
                + unique_prefix_support * 0.06
                + fuzzy_support * 0.04
                + alias_surface_support * 0.14
                + span_similarity * 0.11
                + local_similarity * 0.1
                + full_similarity * 0.07
                + thread_similarity * 0.04
                + min(candidate.confidence, 1.0) * 0.05
                + source_bonus
                + contextual_support
                + (signal.overlay_support if signal is not None else 0.0)
                + layer_bonus
                + entity_kind_bonus
                + type_bonus
                - concept_penalty
            ...
            features = self._entity_link_features(
                ...
                contextual_alias_support=contextual_alias_support,
                unique_prefix_support=unique_prefix_support,
                ...
                thread_similarity=thread_similarity,
                candidate_confidence=min(candidate.confidence, 1.0),
                source_bonus=source_bonus,
                contextual_support=contextual_support,
                overlay_support=(signal.overlay_support if signal is not None else 0.0),
                layer_bonus=layer_bonus,
                entity_kind_bonus=entity_kind_bonus,
                type_bonus=type_bonus,
                concept_penalty=concept_penalty,
                generic_penalty=generic_penalty,
                anti_confusion_penalty=anti_confusion_penalty,
                ...
                target_eligible=bundle.target_eligible,
            )
            verifier_score = self._entity_verifier_score(features)
            final_score = (
                (verifier_score * 0.62) + (score * 0.38)
                if self.verification_level in {"b", "c"}
                else score
            )
            if self.verification_level == "c":
                semantic_consensus = (span_similarity * 0.34) + (local_similarity * 0.3) + (full_similarity * 0.2) + (thread_similarity * 0.16)
                if semantic_consensus < 0.54 and alias_exact < 0.99 and compact_exact < 0.9:
                    final_score -= 0.04
            ranked.append(
                SimilarityMatch(
                    candidate_id=bundle.canonical_entity_id,
                    score=round(max(min(final_score, 0.995), 0.0), 6),
                    candidate_text=bundle.label,
                    metadata={
                        **features,
                        "span_similarity": round(span_similarity, 6),
                        "local_similarity": round(local_similarity, 6),
                        "full_similarity": round(full_similarity, 6),
                        "thread_similarity": round(thread_similarity, 6),
                        "normalized_alias": round(normalized_alias, 6),
                        "boundary_support": round(boundary_support, 6),
                        "contextual_alias_support": round(contextual_alias_support, 6),
                        "unique_prefix_support": round(unique_prefix_support, 6),
                        "fuzzy_support": round(fuzzy_support, 6),
                        "alias_surface_support": round(alias_surface_support, 6),
                        "source_bonus": round(source_bonus, 6),
                        "contextual_support": round(contextual_support, 6),
                        "layer_bonus": round(layer_bonus, 6),
                        "anti_confusion_penalty": round(anti_confusion_penalty, 6),
                        "candidate_entity_type": bundle.entity_type,
                        "candidate_entity_kind": bundle.entity_kind,
                        "type_consistent": type_consistent,
                        "heuristic_score": round(score, 6),
                        "verifier_score": round(verifier_score, 6),
                    },
    ...
    def _mention_worthiness_features(
        self,
        candidate: EntityCandidate,
        normalized_candidate: str,
        *,
        ...
        strongest_alias = max(
            (
                max(
                    signal.exact_alias,
                    signal.normalized_alias,
                    signal.compact_alias,
                    signal.boundary_support,
                    signal.contextual_support,
                    signal.unique_prefix_support,
                    signal.fuzzy_support,
                )
        ...
        return {
            "specificity": specificity,
            "token_count": min(float(token_count) / 4.0, 1.0),
            "has_digits": 1.0 if any(character.isdigit() for character in normalized_candidate) else 0.0,
            "candidate_confidence": min(candidate.confidence, 1.0),
            "strongest_alias_signal": strongest_alias,
            "prototype_top_score": prototype_top_score,
            "is_high_signal_discovery": 1.0 if any(method in _HIGH_SIGNAL_DISCOVERY for method in candidate.discovered_by) else 0.0,
            "is_low_precision_only": 1.0 if candidate.discovered_by and all(method in _LOW_PRECISION_DISCOVERY for method in candidate.discovered_by) else 0.0,
            "type_brand_like": 1.0 if candidate.entity_type_hint in {"brand", "organization", "retailer"} else 0.0,
            "type_product_like": 1.0 if candidate.entity_type_hint in {"product", "product_line"} else 0.0,
            "type_concept": 1.0 if candidate.entity_type_hint == "concept" else 0.0,
            "distinctive_surface": 1.0 if self._looks_distinctive_surface(candidate.text) else 0.0,
        }

    def _entity_link_features(
        self,
        *,
        alias_exact: float,
        normalized_alias: float,
        compact_alias: float,
        boundary_support: float,
        contextual_alias_support: float,
        unique_prefix_support: float,
        fuzzy_support: float,
        alias_surface_support: float,
        span_similarity: float,
        local_similarity: float,
        full_similarity: float,
        thread_similarity: float,
        candidate_confidence: float,
        source_bonus: float,
        contextual_support: float,
        overlay_support: float,
        layer_bonus: float,
        entity_kind_bonus: float,
        type_bonus: float,
        concept_penalty: float,
        generic_penalty: float,
        anti_confusion_penalty: float,
        specificity: float,
        ...
        return {
            ...
            "anti_confusion_penalty": anti_confusion_penalty,
            "specificity": specificity,
            "type_consistent": 1.0 if type_consistent else 0.0,
            "entity_is_concept": 1.0 if entity_kind == "concept" else 0.0,
            "entity_is_product": 1.0 if entity_type in {"product", "product_line"} else 0.0,
            "entity_is_brand_like": 1.0 if entity_type in {"brand", "organization", "retailer"} else 0.0,
            "target_eligible": 1.0 if target_eligible else 0.0,
        }

    def _entity_verifier_score(self, features: dict[str, float]) -> float:
        semantic_consensus = (
            features["span_similarity"] * 0.34
            + features["local_similarity"] * 0.28
            + features["full_similarity"] * 0.2
            + features["thread_similarity"] * 0.12
        )
        surface_support = max(
            features["alias_exact"],
            features["normalized_alias"],
            features["compact_alias"],
            features["boundary_support"],
            features["contextual_alias_support"],
            features["unique_prefix_support"],
            features["fuzzy_support"],
            features["alias_surface_support"],
        )
        anchor_support = min(
            features["source_bonus"]
            + features["contextual_support"]
            + features["layer_bonus"]
            + features["entity_kind_bonus"]
            + features["overlay_support"],
            0.32,
        )
        score = (
            semantic_consensus * 0.34
            + surface_support * 0.34
            + features["candidate_confidence"] * 0.05
            + features["specificity"] * 0.08
            + anchor_support
            + max(features["type_bonus"], 0.0) * 0.08
            + features["target_eligible"] * 0.04
            - max(-features["type_bonus"], 0.0) * 0.22
            - features["concept_penalty"] * 0.72
            - features["generic_penalty"] * 0.8
            - features["anti_confusion_penalty"] * 0.92
        )
        if self.verification_level == "c":
            score += max(surface_support - 0.78, 0.0) * 0.12
            score += max(semantic_consensus - 0.72, 0.0) * 0.12
            if features["entity_is_concept"] > 0.0 and surface_support < 0.82:
                score -= 0.05
            if features["type_consistent"] <= 0.0:
                score -= 0.05
        return round(max(min(score, 0.995), 0.0), 6)

    def _surface_specificity(self, candidate: EntityCandidate, normalized_candidate: str) -> float:
    ...
    def _looks_distinctive_surface(self, text: str) -> bool:
        stripped = text.strip()
        if len(stripped) < 4:
            return False
        return any(character.isupper() for character in stripped) or any(character.isdigit() for character in stripped)

    def _alias_surface_support(self, normalized_candidate: str, candidate_id: str) -> float:
        best_score = 0.0
        candidate_tokens = _significant_tokens(normalized_candidate)
        for alias in self._prefilter_aliases(normalized_candidate):
            ...
            boundary_bonus = 0.0
            if boundary_contains(normalized_candidate, alias.normalized_alias) or boundary_contains(
    ...
    def _evidence_span(self, candidate: EntityCandidate) -> tuple[int, int] | None:
    ...
    def _ensure_vector_index(self) -> None:
        ...
        expected = VectorNamespaceExpectation(
            ...
            backend=self.vector_index.provenance.provider_name,
            dimension=len(items[0].vector),
            normalization_mode="cosine_normalized",
            embedding_model_id=self.embedding_provider.provenance.model_id,
            embedding_provider_name=self.embedding_provider.provenance.provider_name,
            embedding_provider_version=self.embedding_provider.provenance.provider_version,
            embedding_purpose=EmbeddingPurpose.PASSAGE.value,
        ...
        if (
            info is not None
            and info.reuse_state == VectorReuseState.VALID.value
            and self.vector_index.load(namespace=self.vector_namespace, expected=expected)
        ):
            self._candidate_index_ready = True
            return
        self._build_vector_index(items)

    def _vector_items(self) -> list[VectorItem]:
        ...
        vectors = self.embedding_provider.embed_texts(
            [alias_text for _, (_, alias_text, _) in alias_items],
        ...
        return [
            ...
            for (alias_id, (canonical_entity_id, alias_text, alias_source)), vector in zip(
                alias_items,
                vectors,
                strict=True,
            )
    ...
    def _build_vector_index(self, items: list[VectorItem] | None = None) -> None:
        if self.vector_index is None or not self._vector_alias_strings:
            return
    ...
    def vector_items(self) -> list[VectorItem]:
    ...
    def _rank_candidate_pool(
        ...
        for query in query_variants:
            if self.vector_index is None or not self._candidate_index_ready:
                ranked = self.embedding_provider.rank_candidates(
                    query,
                    candidate_texts,
                    purpose=EmbeddingPurpose.LINKING,
                    top_k=min(8, len(candidate_texts)),
                )
            else:
                ...
                hits = self.vector_index.search(
                    query_vector,
                    namespace=self.vector_namespace,
                    top_k=max(24, min(len(self._vector_alias_strings), len(candidate_texts) * 6)),
                )
            ...
            for match in ranked:
                ...
                if existing is None or score > existing.score:
                    aggregated[match.candidate_id] = SimilarityMatch(
                        candidate_id=match.candidate_id,
                        score=score,
                        candidate_text=match.candidate_text,
                        metadata={**match.metadata, "query_variant": query},
                    )
    ...
    def _rank_hits_by_canonical_entity(
        self,
        ...
        for hit in hits:
            ...
            if existing is None or score > existing.score:
                grouped[canonical_entity_id] = SimilarityMatch(
                    candidate_id=canonical_entity_id,
                    score=score,
                    candidate_text=candidate_texts[canonical_entity_id],
                    metadata={
                        **hit.metadata,
                        "matched_alias_text": hit.text,
                    },
                )
    ...
    def _unresolved(
        ...
        *,
        reason: str,
        candidate_canonical_ids: list[str] | None = None,
    ) -> ResolutionDecision:
        return ResolutionDecision(
            source_uap_id=candidate.source_uap_id,
            mention_id=candidate.mention_id,
            candidate_id=candidate.candidate_id,
            candidate_text=candidate.text,
            matched_by="unresolved",
            confidence=0.0,
            evidence_span=self._evidence_span(candidate),
            provider_version=self.embedding_provider.version,
            candidate_canonical_ids=candidate_canonical_ids or [],
            unresolved_reason=reason,
        )

    def _prototype_context_support(self, bundle: object, normalized_context: str) -> float:
        if not normalized_context:
            return 0.0
        phrase_matches = sum(
            1
            for phrase in getattr(bundle, "related_phrases", ())[:8]
            if phrase and phrase in normalized_context
        )
        anchor_matches = sum(
            1
            for phrase in getattr(bundle, "domain_anchor_phrases", ())[:8]
            if phrase and phrase in normalized_context
        )
        neighboring_matches = sum(
            1
            for phrase in (
                *getattr(bundle, "neighboring_entity_labels", ())[:6],
                *getattr(bundle, "neighboring_aspect_labels", ())[:6],
                *getattr(bundle, "neighboring_issue_labels", ())[:6],
            )
            if phrase and normalize_alias(phrase) in normalized_context
        )
        topic_matches = sum(
            1
            for phrase in getattr(bundle, "related_topic_labels", ())[:4]
            if phrase and normalize_alias(phrase) in normalized_context
        )
        score = (phrase_matches * 0.03) + (anchor_matches * 0.035) + (neighboring_matches * 0.02) + (topic_matches * 0.025)
        return round(min(score, 0.18), 6)

    def _embedding_result_is_ambiguous(
        self,
        candidate: EntityCandidate,
        ranked_matches: list[SimilarityMatch],
    ) -> bool:
    ...
    def _decision_for_entity(
        ...
        *,
        ...
        return ResolutionDecision(
            ...
            resolution_kind="concept" if concept_entity_id is not None else "canonical_entity",
            resolved_entity_kind="concept" if concept_entity_id is not None else "entity",
            knowledge_layer=entity.knowledge_layer,
            candidate_canonical_ids=candidate_canonical_ids,
        )

    def _build_alias_token_index(
        self,
    ...
    def _prefilter_aliases(self, normalized_candidate: str) -> tuple[EntityAlias, ...]:
        ...
        return tuple(
            ...
            if abs(len(alias.normalized_alias) - len(normalized_candidate)) <= 12
        ) or self._all_aliases

    def _build_compact_alias_index(
        self,
        aliases: tuple[EntityAlias, ...],
    ) -> dict[str, list[EntityAlias]]:
        index: dict[str, list[EntityAlias]] = {}
    ...
    def _entity_compact_aliases(self, canonical_entity_id: str) -> set[str]:
        return {
            alias.normalized_alias.replace(" ", "")
            for alias in self._all_aliases
