...
def cluster_unresolved_entity_facts(
    ...
    grouped: dict[tuple[str, str | None], list[EntityFact]] = defaultdict(list)
    ...
    cluster_records: list[EntityCandidateClusterFact] = []
    cluster_info: dict[tuple[str, str | None], tuple[str, int]] = {}
    for (normalized_surface, entity_type), facts in grouped.items():
        ...
        cluster_id = "candclu." + hashlib.sha256(
        ...
        languages = sorted(
            {
                mention.language
                for mention_id in unique_mentions
                if (mention := mention_by_id.get(mention_id)) is not None and mention.language
            }
        )
        ...
        promotion_state: Literal["reviewable", "promotable", "promoted"] = (
            "promotable" if cluster_size >= 3 else "reviewable"
        )
        ...
        cluster_records.append(
            EntityCandidateClusterFact(
                ...
                mention_count=cluster_size,
                source_languages=languages,
                discovered_by=discovered_by,
                representative_mention_ids=unique_mentions[:5],
                candidate_canonical_ids=candidate_ids,
                promotion_state=promotion_state,
                provenance=FactProvenance(
                    source_uap_id=provenance_fact.source_uap_id,
                    mention_id=provenance_fact.mention_id,
                    provider_version=provenance_fact.provenance.provider_version,
                    rule_version="batch-candidate-clustering-v1",
                    evidence_text=representative_surface,
                    evidence_span=provenance_fact.provenance.evidence_span,
                ),
            )
        )

    updated_facts: list[EntityFact] = []
    for fact in entity_facts:
        normalized_surface = normalize_alias(fact.candidate_text)
        ...
        updated_facts.append(
            fact.model_copy(
                update={
                    ...
                    "unresolved_cluster_size": cluster_size,
                }
            )
        )
