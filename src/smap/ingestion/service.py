...
from smap.validation.models import (
    BatchProfile,
    FieldProfile,
    ValidationErrorItem,
    ValidationReport,
...
def _present_count(field_stats: dict[str, _FieldStats], *field_names: str) -> int:
    return max((field_stats.get(field_name, _FieldStats()).present for field_name in field_names), default=0)
...
def ingest_batch(
    ...
    *,
    ...
    records_by_platform: Counter[str] = Counter()
    field_stats: dict[str, _FieldStats] = defaultdict(_FieldStats)
    ...
    output_handle_context = (
        raw_output_path.open("w", encoding="utf-8")
        if raw_output_path is not None
        else nullcontext()
    )
    ...
    if validation_report.total_records and _present_count(field_stats, "content.keywords", "content.tiktok_keywords") < validation_report.total_records:
        notes.append("Semantic keyword fields are sparse and must remain optional.")

    batch_profile = BatchProfile(
        total_records=validation_report.total_records,
        records_by_type=counter_to_dict(records_by_type),
        records_by_platform=counter_to_dict(records_by_platform),
        field_profiles=[
            FieldProfile(
        ...
        text_length={
            "avg": round(mean(text_lengths), 2) if text_lengths else 0.0,
            "min": float(min(text_lengths)) if text_lengths else 0.0,
            "max": float(max(text_lengths)) if text_lengths else 0.0,
        },
        notes=notes,
    )
    ...
    return IngestedBatchBundle(
        ...
        input_inspection=inspect_input_path(input_path),
        materialized_raw_jsonl_path=str(raw_output_path) if raw_output_path is not None else None,
        raw_record_count=validation_report.total_records,
    )
