...
def build_bi_reports(
    ...
    *,
    ...
    emerging_topics_report = _build_emerging_topics_report(
        bundle,
        contracts["emerging_topics_report"],
    ...
    thread_controversy_report = _build_thread_controversy_report(
        bundle,
        contracts["thread_controversy_report"],
        window_context.window,
    )
...
def _window_context(fact_mentions: pl.DataFrame) -> _WindowContext:
    ...
    return _WindowContext(
        window=ReportWindow(
            analysis_start=analysis_start,
            analysis_end=analysis_end,
            delta_current_start=_iso(current["posted_at"].to_list()[0]),
            delta_current_end=_iso(current["posted_at"].to_list()[-1]),
            delta_previous_start=_iso(previous["posted_at"].to_list()[0]),
            delta_previous_end=_iso(previous["posted_at"].to_list()[-1]),
    ...
    *,
    ...
    if not resolved.is_empty():
        full_grouped = (
            ...
            .sort(["mention_count", "entity_hit_count", "canonical_entity_id"], descending=[True, True, False])
        )
        ...
        for row in full_grouped.to_dicts():
            ...
            rows.append(
                SOVRow(
                    ...
                    effective_mention_share=_ratio(effective_count, effective_mention_volume),
                    ...
                    delta_mention_count=int(current_row["mention_count"] - previous_row["mention_count"]),
                    delta_mention_share=round(
                        _ratio(current_effective_count, max(sum(current_effective.values()), 1.0))
                        - _ratio(previous_effective_count, max(sum(previous_effective.values()), 1.0)),
                        4,
                    ),
                    delta_entity_hit_count=int(current_row["entity_hit_count"] - previous_row["entity_hit_count"]),
    ...
    rows.sort(
        key=lambda item: (
            -item.effective_mention_share if weighting_mode != "raw" else -item.mention_share,
            -item.entity_hit_share,
            item.canonical_entity_id,
        )
    )
    unresolved_mentions = mention_volume - (
        int(resolved.select(pl.col("mention_id").n_unique()).item()) if not resolved.is_empty() else 0
    )
        ...
        for row in grouped.to_dicts():
            ...
            entity_rows.append(
                BuzzEntityRow(
                    ...
                    buzz_score_proxy=round(effective_count * (1.0 + max(growth_ratio, 0.0)), 4),
                    evidence_references=_head_mentions(
                        resolved.filter(pl.col("canonical_entity_id") == entity_id),
                        limit=5,
                    ),
    ...
    if not fact_topics.is_empty():
        ...
        for row in grouped.to_dicts():
            ...
            topic_row = BuzzTopicRow(
                ...
                label_source=str(artifact.get("label_source")) if artifact and artifact.get("label_source") is not None else None,
                label_health_score=_optional_float(artifact.get("label_health_score")) if artifact else None,
                reporting_status=str(artifact.get("reporting_status") or "reportable") if artifact else "reportable",
                raw_topic_keys=[
                    value
                ...
                canonical_evidence_phrases=[
                    value
                    for value in cast(list[object], artifact.get("canonical_evidence_phrases") or [])
                    if isinstance(value, str)
                ]
                if artifact
                else [],
                ...
                supporting_phrases=[
                    value
                    for value in cast(list[object], artifact.get("supporting_phrases") or [])
                    if isinstance(value, str)
                ]
                if artifact
                else [],
                ...
                growth_ratio_proxy=growth_ratio,
                buzz_score_proxy=round(effective_count * (1.0 + max(growth_ratio, 0.0)), 4),
                quality_score=_optional_float(artifact.get("quality_score")) if artifact else None,
                usefulness_score=_optional_float(artifact.get("usefulness_score")) if artifact else None,
                reportability_score=_optional_float(artifact.get("reportability_score")) if artifact else None,
                artifact_purity_score=_optional_float(artifact.get("artifact_purity_score")) if artifact else None,
                ...
                evidence_references=_head_mentions(
                    reporting_topics.filter(pl.col("reporting_topic_key") == reporting_key),
                    limit=5,
                ),
            )
...
def _build_emerging_topics_report(
    ...
    if not fact_topic_artifacts.is_empty():
        ...
        for row in emerging.to_dicts():
            topics.append(
                EmergingTopicRow(
                    reporting_topic_key=row.get("reporting_topic_key") or row["topic_key"],
                    reporting_topic_label=row.get("reporting_topic_label") or row["topic_label"],
                    label_source=str(row.get("label_source")) if row.get("label_source") is not None else None,
                    ...
                    growth_delta=float(row["growth_delta"]) if row.get("growth_delta") is not None else None,
                    stability_score=float(row["stability_score"]) if row.get("stability_score") is not None else None,
                    quality_score=float(row["quality_score"]) if row.get("quality_score") is not None else None,
                    usefulness_score=float(row["usefulness_score"]) if row.get("usefulness_score") is not None else None,
                    reportability_score=float(row["reportability_score"]) if row.get("reportability_score") is not None else None,
                    artifact_purity_score=float(row["artifact_purity_score"]) if row.get("artifact_purity_score") is not None else None,
                    ...
                    canonical_evidence_phrases=list(row.get("canonical_evidence_phrases") or []),
                    salient_terms=list(row.get("salient_terms") or []),
                    supporting_phrases=list(row.get("supporting_phrases") or []),
                    representative_document_ids=list(row.get("representative_document_ids") or []),
        ...
        for issue_category, issue_frame in valid.partition_by("issue_category", as_dict=True).items():
            ...
            issues.append(
                TopIssueRow(
                    ...
                    growth_delta_mentions=current_counts.get(str(issue_label), 0) - previous_counts.get(str(issue_label), 0),
...
def _build_thread_controversy_report(
    ...
    if not fact_threads.is_empty():
        ...
        issue_roots = (
            fact_issue_signals.select(["mention_id", "issue_category"])
            .join(mention_roots.select(["mention_id", "root_id"]), on="mention_id", how="left")
            if not fact_issue_signals.is_empty()
            else pl.DataFrame({"mention_id": [], "issue_category": [], "root_id": []})
        )
        sentiment_roots = (
            sentiments.join(mention_roots.select(["mention_id", "root_id"]), on="mention_id", how="left")
        ...
        for row in fact_threads.to_dicts():
            ...
            author_diversity = _ratio(
                int(thread_mentions.select(pl.col("author_id").n_unique()).item()) if not thread_mentions.is_empty() else 0,
            ...
            issue_diversity = _ratio(
                int(thread_issues.select(pl.col("issue_category").n_unique()).item()) if not thread_issues.is_empty() else 0,
            ...
            threads.append(
                ThreadControversyRow(
                    ...
                    evidence_references=[root_id, *list(row.get("top_comment_ids") or [])],
                )
            )
        ...
        for row in (
            ...
            creators.append(
                CreatorBreakdownRow(
                    ...
                    top_topic_keys=_top_values(topic_frame.filter(pl.col("mention_id").is_in(mention_ids)), "reporting_topic_key"),
                    top_issue_categories=_top_values(
                        fact_issue_signals.filter(pl.col("mention_id").is_in(mention_ids)),
                        "issue_category",
                    ),
                    controversy_thread_count=sum(
                        1
                        for root_id in set(fact_mentions.filter(pl.col("author_id") == creator_id)["root_id"].to_list())
            ...
            for row in (
                ...
                thread_initiators.append(
                    ThreadInitiatorRow(
                        ...
                        share_of_threads=_ratio(int(row["thread_count"]), total_threads),
                    )
                )
            ...
            sources.append(
                SourceBreakdownRow(
                    ...
                    top_topic_keys=_top_values(
                        _topic_reporting_frame(fact_topics).filter(pl.col("mention_id").is_in(mention_ids)),
                        "reporting_topic_key",
                    ),
                    top_issue_categories=_top_values(
                        fact_issue_signals.filter(pl.col("mention_id").is_in(mention_ids)),
        ...
        if top_entity.delta_mention_count != 0:
            ...
            cards.append(
                InsightCard(
                    ...
                    filters_used={"canonical_entity_id": top_entity.canonical_entity_id},
    ...
    if top_issues_report.issues:
        ...
        cards.append(
            InsightCard(
                ...
                confidence=min(0.58 + (issue.issue_pressure_proxy / max(issue.issue_pressure_proxy, 1.0)) * 0.18, 0.86),
                time_window=time_window,
...
def _entity_counts(frame: pl.DataFrame, *, mention_ids: set[str]) -> dict[str, dict[str, int]]:
...
def _effective_mention_volume(fact_mentions: pl.DataFrame, weighting_mode: str) -> float:
...
def _entity_unresolved_reason_breakdown(frame: pl.DataFrame) -> dict[str, int]:
    ...
    unresolved = frame.filter(
        pl.col("canonical_entity_id").is_null() & pl.col("concept_entity_id").is_null() & pl.col("unresolved_reason").is_not_null()
    )
    ...
    return {
        ...
        for row in grouped.sort(["mention_count", "unresolved_reason"], descending=[True, False]).to_dicts()
    }
...
def _topic_reporting_frame(frame: pl.DataFrame) -> pl.DataFrame:
    ...
    working = frame.with_columns(
        pl.coalesce([pl.col("reporting_topic_key"), pl.col("effective_topic_key"), pl.col("topic_key")]).alias("reporting_topic_key"),
        pl.coalesce([pl.col("reporting_topic_label"), pl.col("effective_topic_label"), pl.col("topic_label")]).alias("reporting_topic_label"),
    )
...
def _issue_pressure_proxy(frame: pl.DataFrame) -> float:
    ...
    for row in working.select(
        ["severity", "evidence_mode", "confidence", "target_grounding_confidence", "corroboration_confidence"]
    ).to_dicts():
        ...
        trust_multiplier = 0.35 + (
            0.65 * ((confidence_multiplier + grounding_multiplier + corroboration_multiplier) / 3.0)
        )
        total += (
...
def _entity_name(dimensions: dict[str, dict[str, object]], entity_id: str) -> str | None:
    row = dimensions.get(entity_id, {})
    value = row.get("entity_name")
    return str(value) if isinstance(value, str) else None


def _entity_type(dimensions: dict[str, dict[str, object]], entity_id: str) -> str | None:
    row = dimensions.get(entity_id, {})
