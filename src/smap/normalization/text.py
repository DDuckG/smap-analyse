...
def normalize_hashtags(text: str, explicit_hashtags: list[str] | None = None) -> list[str]:
...
def explicit_language_result(explicit_language: str) -> LanguageIdentificationResult:
...
def normalize_social_text(
    ...
    *,
    ...
    language_result = resolve_language(
        raw_text,
    ...
    language_label, language_supported, rejection_reason, gate_metadata = _language_gate(
        raw_text,
    ...
    return TextNormalizationResult(
        ...
        language_confidence=round(
            max(language_result.confidence, 0.72 if language_label in {"vi", "en", "mixed"} else 0.0),
            4,
        ),
        ...
        language_source=language_result.source.value,
        language_metadata={
            **{
                key: value
        ...
        semantic_route_hint="semantic_skip" if not language_supported else "semantic_full",
    )
...
def _normalize_processing_text(raw_text: str, hashtags: list[str]) -> str:
    ...
    for slang, replacement in _SOCIAL_SLANG_MAP.items():
        normalized = re.sub(rf"(?<!\w){re.escape(slang)}(?!\w)", replacement, normalized, flags=re.IGNORECASE)
...
def _segment_hashtag(hashtag: str) -> str:
    segmented = re.sub(r"([a-z])([A-Z])", r"\1 \2", hashtag).replace("_", " ").replace("-", " ")
...
def _language_gate(
    raw_text: str,
    compact_text: str,
    provider_result: LanguageIdentificationResult,
) -> tuple[str, bool, str | None, dict[str, str | int | float | bool | None]]:
    ...
    mixed_vi_evidence = vi_score >= 0.18 or vi_semantic_hits >= 2 or any(
        character in raw_text.casefold() for character in _VI_DIACRITIC_MARKERS
    )
...
def _looks_supported_short_form(token: str) -> bool:
