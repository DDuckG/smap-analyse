...
from smap.review.knowledge_state_hashing import (
    canonical_review_alias_contribution,
    canonical_review_noise_term,
    stable_content_hash,
)
...
class RelevantSemanticRegion(BaseModel):
    ...
    @classmethod
    def from_signature(cls, signature: SemanticSignature) -> RelevantSemanticRegion:
        return cls(
    ...
    @property
    def is_entity_like(self) -> bool:
        return self.problem_class in {
            ReviewProblemClass.UNRESOLVED_ENTITY_CANDIDATE,
            ReviewProblemClass.AMBIGUOUS_ENTITY_MAPPING,
            ReviewProblemClass.SUSPICIOUS_NOISE_CANDIDATE,
        }


def relevant_reviewed_knowledge_fingerprint(
    ...
    *,
    ...
    relevant_aliases = _sorted_payload(
        canonical_review_alias_contribution(contribution)
        for contribution in registry.alias_contributions
    if contribution.source == "review" and _alias_is_relevant(contribution, region, context)
    )
    relevant_noise_terms = _sorted_payload(
        canonical_review_noise_term(noise_term)
        for noise_term in registry.noise_terms
    if noise_term.source == "review" and _noise_term_is_relevant(noise_term, region, context)
    )
    ...
    return stable_content_hash(
        {
            "semantic_region": region.model_dump(mode="json"),
            "alias_contributions": relevant_aliases,
            "noise_terms": relevant_noise_terms,
        }
    )


def _alias_is_relevant(
    contribution: AliasContribution,
    region: RelevantSemanticRegion,
    context: ReviewContext,
) -> bool:
    if _normalize_surface(contribution.alias) != region.normalized_candidate_text:
        return False
    if (
        region.entity_type_hint is not None
        and contribution.entity_type is not None
        and contribution.entity_type != region.entity_type_hint
    ):
        return False
...
def _static_scope_matches(scope_payload: object, context: ReviewContext) -> bool:
...
def _base_knowledge_matches_context(
    provenance: Mapping[str, OverlayProvenanceValue],
    context: ReviewContext,
) -> bool:
    applicability_policy = _load_json_object(provenance.get("applicability_policy_json"))
...
def _knowledge_state_matches(knowledge_state: Mapping[str, object], context: ReviewContext) -> bool:
...
def _sorted_payload(items: list[dict[str, object]] | tuple[dict[str, object], ...] | object) -> list[dict[str, object]]:
