...
class AuthorityPrecedencePolicy:
    ...
    def choose(self, *, group_candidate, item_candidate):
        ...
        if (item_candidate.created_at, item_candidate.decision_id) >= (group_candidate.created_at, group_candidate.decision_id):
            return AuthoritySelection(winner=AuthorityLevel.ITEM, reason='later_authority_supersedes')
        return AuthoritySelection(winner=AuthorityLevel.GROUP, reason='broader_authority_retained')

    @staticmethod
    def _scope_specificity_rank(policy):
        return sum((1 for key, value in policy.valid_scope_key.static_scope.model_dump(mode='json').items() if key != 'signature_version' and value is not None))

    @staticmethod
    def _knowledge_specificity_rank(policy):
        order = {'ignore': 0, 'base_ontology': 1, 'base_with_non_review_overlays': 2, 'reviewed_state_sensitive': 3, 'exact_full_state': 4}
        return order[policy.knowledge_match_mode.value]

    @staticmethod
    def _match_specificity_rank(policy):
        order = {'normalized_form': 0, 'classification_label': 1, 'normalized_form_and_type': 2, 'ambiguity_set': 3}
