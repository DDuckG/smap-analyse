...
def _policy_fingerprint(payload: dict[str, Any]) -> str:
...
class ReviewPolicyEngine:
    ...
    def grouping_policy_for(self, signature: SemanticSignature) -> ReviewGroupingPolicy:
        if signature.problem_class == ReviewProblemClass.LOW_CONFIDENCE_CLASSIFICATION:
            return ReviewGroupingPolicy(
                problem_class=signature.problem_class,
                scope_level=ReviewScopeLevel.TASK_ONTOLOGY,
                match_mode=SemanticMatchMode.CLASSIFICATION_LABEL,
                knowledge_match_mode=KnowledgeMatchMode.BASE_WITH_NON_REVIEW_OVERLAYS,
            )
        if signature.problem_class == ReviewProblemClass.AMBIGUOUS_ENTITY_MAPPING:
            return ReviewGroupingPolicy(
                problem_class=signature.problem_class,
                scope_level=ReviewScopeLevel.PROJECT_ONTOLOGY,
                match_mode=SemanticMatchMode.AMBIGUITY_SET,
                knowledge_match_mode=KnowledgeMatchMode.REVIEWED_STATE_SENSITIVE,
            )
        if signature.problem_class == ReviewProblemClass.SUSPICIOUS_NOISE_CANDIDATE:
            return ReviewGroupingPolicy(
                problem_class=signature.problem_class,
                scope_level=ReviewScopeLevel.PROJECT_ONTOLOGY,
                match_mode=SemanticMatchMode.NORMALIZED_FORM_AND_TYPE,
                knowledge_match_mode=KnowledgeMatchMode.REVIEWED_STATE_SENSITIVE,
            )
        return ReviewGroupingPolicy(
            problem_class=signature.problem_class,
    ...
    def scope_key_for(
        ...
        *,
        ...
        knowledge_match_mode = (
            grouping_policy.knowledge_match_mode
            if grouping_policy is not None
            else KnowledgeMatchMode.BASE_WITH_NON_REVIEW_OVERLAYS
        )

        if scope_level in {
            ReviewScopeLevel.PROJECT,
            ReviewScopeLevel.PROJECT_ONTOLOGY,
            ReviewScopeLevel.TASK,
            ReviewScopeLevel.TASK_ONTOLOGY,
            ReviewScopeLevel.THREAD_ONTOLOGY,
        }:
        ...
        if scope_level in {
            ...
            if knowledge_match_mode in {
                KnowledgeMatchMode.BASE_ONTOLOGY,
                KnowledgeMatchMode.BASE_WITH_NON_REVIEW_OVERLAYS,
                KnowledgeMatchMode.REVIEWED_STATE_SENSITIVE,
                KnowledgeMatchMode.EXACT_FULL_STATE,
            }:
            ...
            if knowledge_match_mode in {
                ...
                knowledge_state.reviewed_overlay_fingerprint = (
                    self._grouping_reviewed_knowledge_fingerprint(context=context, signature=signature)
                    if grouping_policy is not None
                    else context.reviewed_overlay_fingerprint
                )
        ...
        return ScopeKey(
            ...
            signature_version=context.signature_version,
        )

    def _grouping_reviewed_knowledge_fingerprint(
        self,
        *,
        context: ReviewContext,
        ...
        if registry is None:
            return "none"
        return relevant_reviewed_knowledge_fingerprint(
            registry,
            signature=signature,
            context=context,
        )

    def _get_ontology_registry(self) -> OntologyRegistry | None:
    ...
    def build_applicability_policy(
        ...
        *,
        ...
        authority_level: AuthorityLevel,
        knowledge_match_mode: KnowledgeMatchMode | None = None,
        ...
        if terminate_future_authority is None:
            effective_terminate_future_authority = (
                future_effect == FutureEffectType.NONE
                and (
                    resolution_scope == ReviewResolutionScope.CLOSE_ONLY.value
                    or authority_level == AuthorityLevel.GROUP
                )
            )
        else:
        ...
        return DecisionApplicabilityPolicy(
            ...
            scope_level=scope_key.scope_level,
            authority_level=authority_level,
            future_effect=future_effect,
            terminates_future_authority=effective_terminate_future_authority,
            match_mode=grouping_policy.match_mode,
            knowledge_match_mode=effective_knowledge_match_mode,
            valid_signature_version=signature.signature_version,
            valid_scope_key=scope_key,
            semantic_match_value=signature.match_value(grouping_policy.match_mode),
            semantic_fingerprint=signature.fingerprint(),
            origin_context_fingerprint=context.fingerprint(),
            origin_knowledge_state=context.knowledge_state(),
        )
