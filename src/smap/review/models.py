...
class ReviewDecision(Base):
    ...
    resolution_scope: Mapped[str] = mapped_column(
        String(80),
        default=ReviewResolutionScope.ITEM_ONLY.value,
    )
    review_context_payload: Mapped[dict[str, Any] | None] = mapped_column(JSON, nullable=True)
    semantic_signature_payload: Mapped[dict[str, Any] | None] = mapped_column(JSON, nullable=True)
    ...
    supersedes_review_decision_id: Mapped[int | None] = mapped_column(
        ForeignKey("review_decisions.review_decision_id"),
        nullable=True,
        index=True,
    )
    terminates_future_authority: Mapped[bool] = mapped_column(default=False)
    terminated_by_review_decision_id: Mapped[int | None] = mapped_column(
        ForeignKey("review_decisions.review_decision_id"),
        nullable=True,
        index=True,
    )
    origin_group_decision_id: Mapped[int | None] = mapped_column(
        ForeignKey("review_group_decisions.review_group_decision_id"),
        nullable=True,
        index=True,
    )
    effect_applied_from_group_decision_id: Mapped[int | None] = mapped_column(
        ForeignKey("review_group_decisions.review_group_decision_id"),
        nullable=True,
        index=True,
    )
...
class ReviewGroupDecision(Base):
    ...
    applicability_fingerprint: Mapped[str | None] = mapped_column(String(255), nullable=True, index=True)
    terminates_future_authority: Mapped[bool] = mapped_column(default=False)
    supersedes_review_group_decision_id: Mapped[int | None] = mapped_column(
        ForeignKey("review_group_decisions.review_group_decision_id"),
        nullable=True,
        index=True,
    )
    terminated_by_review_group_decision_id: Mapped[int | None] = mapped_column(
        ForeignKey("review_group_decisions.review_group_decision_id"),
        nullable=True,
        index=True,
    )
