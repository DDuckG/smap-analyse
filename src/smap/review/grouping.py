...
def list_review_groups(
    ...
    *,
    ...
    query: Select[tuple[ReviewGroup]] = (
        select(ReviewGroup)
        .options(selectinload(ReviewGroup.items), selectinload(ReviewGroup.decisions))
        .order_by(ReviewGroup.updated_at.desc())
        .offset(offset)
        .limit(limit)
    )
    if status:
...
def get_review_group(session: Session, review_group_id: int) -> ReviewGroup:
    group = session.scalar(
        select(ReviewGroup)
        .options(selectinload(ReviewGroup.items), selectinload(ReviewGroup.decisions))
        .where(ReviewGroup.review_group_id == review_group_id)
    )
    if group is None:
...
def count_open_review_groups(session: Session) -> int:
    result = session.scalar(
        select(func.count()).select_from(ReviewGroup).where(ReviewGroup.status.in_(OPEN_REVIEW_STATUSES))
    )
    return int(result or 0)


def list_group_decisions(session: Session, review_group_id: int) -> list[ReviewGroupDecision]:
    return list(
        session.scalars(
            select(ReviewGroupDecision)
            .where(ReviewGroupDecision.review_group_id == review_group_id)
            .order_by(ReviewGroupDecision.created_at.asc(), ReviewGroupDecision.review_group_decision_id.asc())
        )
    )


def latest_group_decision(session: Session, review_group_id: int) -> ReviewGroupDecision | None:
    return session.scalar(
        select(ReviewGroupDecision)
        .where(ReviewGroupDecision.review_group_id == review_group_id)
        .order_by(ReviewGroupDecision.created_at.desc(), ReviewGroupDecision.review_group_decision_id.desc())
        .limit(1)
    )


def effective_future_group_decision(
    session: Session,
    review_group_id: int,
    *,
    applicability_engine: ApplicabilityEngine | None = None,
) -> ReviewGroupDecision | None:
...
def find_group_by_fingerprint(session: Session, group_fingerprint: str) -> ReviewGroup | None:
    return session.scalar(
        select(ReviewGroup)
        .options(selectinload(ReviewGroup.decisions))
        .where(ReviewGroup.review_signature == group_fingerprint)
    )
...
def find_candidate_groups(
    ...
    *,
    ...
    query = (
        select(ReviewGroup)
        .options(selectinload(ReviewGroup.decisions))
        .where(ReviewGroup.problem_class == problem_class)
        .where(ReviewGroup.semantic_fingerprint == semantic_fingerprint)
    )
    return list(session.scalars(query))


def get_group_context(group: ReviewGroup) -> ReviewContext | None:
    if group.review_context_payload is None:
        return None
    return ReviewContext.model_validate(group.review_context_payload)


def get_group_signature(group: ReviewGroup) -> SemanticSignature | None:
    if group.semantic_signature_payload is None:
        return None
    return SemanticSignature.model_validate(group.semantic_signature_payload)


def get_group_scope_key(group: ReviewGroup) -> ScopeKey | None:
    if group.scope_key_payload is None:
        return None
    return ScopeKey.model_validate(group.scope_key_payload)


def get_grouping_policy(group: ReviewGroup) -> ReviewGroupingPolicy | None:
    if group.grouping_policy_payload is None:
        return None
    return ReviewGroupingPolicy.model_validate(group.grouping_policy_payload)


def get_applicability_policy(decision: ReviewGroupDecision) -> DecisionApplicabilityPolicy | None:
...
def has_applicable_future_decision(
    session: Session,
    *,
    ...
    for group in candidate_groups:
        decision = effective_future_group_decision(
            session,
...
def ensure_review_group(
    session: Session,
    *,
    context: ReviewContext,
    signature: SemanticSignature,
    scope_key: ScopeKey,
    grouping_policy: ReviewGroupingPolicy,
    ...
    if group is None:
        group = ReviewGroup(
            ...
            grouping_policy_payload=grouping_policy.model_dump(mode="json"),
            ...
            candidate_canonical_ids=signature.candidate_canonical_ids,
...
def attach_review_item_to_group(
    session: Session,
...
def register_suppressed_occurrence(session: Session, group: ReviewGroup) -> None:
    group.occurrence_count += 1
    group.last_seen_at = utc_now()
    session.add(group)


def assign_review_group(session: Session, review_group_id: int, assignee: str) -> ReviewGroup:
...
def rebuild_review_groups(
    session: Session,
    policy_engine: ReviewPolicyEngine | None = None,
) -> int:
    active_items = list(
        session.scalars(
            select(ReviewItem).where(ReviewItem.status.in_(OPEN_REVIEW_STATUSES)).order_by(ReviewItem.created_at.asc())
        )
    )
    ...
    for item in active_items:
        ...
        scope_key = (
            ScopeKey.model_validate(scope_key_payload)
            if scope_key_payload
            else engine.scope_key_for(
                context,
        ...
        group = ensure_review_group(
            ...
            scope_key=scope_key,
            grouping_policy=grouping_policy,
            representative_payload=item.payload,
            group_fingerprint=engine.review_group_fingerprint(signature, scope_key),
            applicability_engine=applicability_engine,
        )
...
def iter_group_candidate_texts(group: ReviewGroup) -> Iterable[str]:
    seen: set[str] = set()
    for item in group.items:
