...
def build_entity_signature(entity_fact: EntityFact) -> SemanticSignature:
    ...
    return SemanticSignature(
        ...
        entity_type_hint=entity_fact.entity_type,
        ambiguity_signature=ambiguity_signature,
        candidate_canonical_ids=sorted(entity_fact.canonical_candidate_ids),
    )


def build_classification_signature(
    fact: SentimentFact | IntentFact | StanceFact,
) -> SemanticSignature:
    ...
    return SemanticSignature(
        ...
        problem_class=ReviewProblemClass.LOW_CONFIDENCE_CLASSIFICATION,
        label_key=label_key,
    )


def build_signature_from_payload(item_type: str, payload: dict[str, Any]) -> SemanticSignature:
    if item_type == "entity":
        ...
        candidate_canonical_ids = (
            sorted(str(item) for item in canonical_ids_raw)
            if isinstance(canonical_ids_raw, list)
            else []
        )
        ...
        return SemanticSignature(
            ...
            problem_class=problem_class,
            normalized_candidate_text=normalized_candidate_text,
            entity_type_hint=str(payload.get("entity_type")) if payload.get("entity_type") else None,
            ambiguity_signature=ambiguity_signature,
