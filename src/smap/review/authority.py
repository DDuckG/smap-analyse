...
class AuthorityResolver:
    ...
    def resolve_group_authority(self, decisions):
        if not decisions:
        ...
        return GroupAuthorityResolution(authoritative_decision=self._current_group_authority(future_decisions, superseded_ids, terminated_ids), superseded_ids=superseded_ids, terminated_ids=terminated_ids, terminating_decision_id=terminating_decision_id, reason=self._group_reason(future_decisions, superseded_ids, terminated_ids, terminating_decision_id))

    def resolve_item_authority(self, *, group_decisions, item_decisions):
        ...
        if authoritative_item is not None:
            ...
            if authoritative_group is not None:
                ...
                if group_policy is not None and item_policy is not None:
                    choice = self.precedence_policy.choose(group_candidate=AuthorityCandidate(authority_level=AuthorityLevel.GROUP, created_at=authoritative_group.created_at, decision_id=authoritative_group.review_group_decision_id, policy=group_policy), item_candidate=AuthorityCandidate(authority_level=AuthorityLevel.ITEM, created_at=authoritative_item.created_at, decision_id=authoritative_item.review_decision_id, policy=item_policy))
                    if choice.winner == AuthorityLevel.GROUP:
                        return ItemAuthorityResolution(source_level=AuthorityLevel.GROUP, authoritative_group_decision=authoritative_group, authoritative_item_decision=authoritative_item, group_resolution=group_resolution, reason=choice.reason)
            return ItemAuthorityResolution(source_level=AuthorityLevel.ITEM, authoritative_group_decision=authoritative_group, authoritative_item_decision=authoritative_item, group_resolution=group_resolution, reason=choice.reason if choice is not None else 'authoritative_future_decision_active')
        if authoritative_group is None:
            return ItemAuthorityResolution(source_level=AuthorityLevel.ITEM if authoritative_item is not None else None, authoritative_item_decision=authoritative_item, group_resolution=group_resolution, reason='future_authority_terminated' if latest_item_terminator is not None else None)
        if latest_item_terminator is not None and latest_item_terminator.created_at >= authoritative_group.created_at:
            return ItemAuthorityResolution(source_level=None, authoritative_group_decision=authoritative_group, group_resolution=group_resolution, item_terminated_group_authority=True, reason='future_authority_terminated')
        return ItemAuthorityResolution(source_level=AuthorityLevel.GROUP, authoritative_group_decision=authoritative_group, authoritative_item_decision=authoritative_item, group_resolution=group_resolution, reason=group_resolution.reason or 'authoritative_future_decision_active')

    @staticmethod
    def _group_reason(future_decisions, superseded_ids, terminated_ids, terminating_decision_id):
    ...
    @staticmethod
    def _current_group_authority(future_decisions, superseded_ids, terminated_ids):
        active = [decision for decision_id, decision in future_decisions.items() if decision_id not in superseded_ids and decision_id not in terminated_ids]
        if not active:
            return None
        return max(active, key=lambda item: (item.created_at, item.review_group_decision_id))

    def _resolve_item_future_authority(self, decisions):
        if not decisions:
            return None
    ...
    def _latest_item_terminator(self, decisions):
    ...
    @staticmethod
    def _current_item_authority(future_decisions, superseded_ids, terminated_ids):
        active = [decision for decision_id, decision in future_decisions.items() if decision_id not in superseded_ids and decision_id not in terminated_ids]
        if not active:
            return None
