...
from smap.review.knowledge_state_hashing import non_review_overlay_fingerprint, ontology_semantic_fingerprint, reviewed_overlay_fingerprint, stable_content_hash
...
def build_ontology_fingerprint(registry):
    return ontology_semantic_fingerprint(registry)

def build_overlay_fingerprint(registry):
    return non_review_overlay_fingerprint(registry)

def build_reviewed_overlay_fingerprint(registry):
...
class ReviewContext(BaseModel):
    ...
    def static_scope_key(self):
        return StaticScopeKey(project_id=self.project_id, task_id=self.task_id, platform=self.platform, root_id=self.root_id, language_family=self.language_family, signature_version=self.signature_version)

    def knowledge_state(self):
        return KnowledgeStateFingerprint(ontology_fingerprint=self.ontology_fingerprint, overlay_fingerprint=self.overlay_fingerprint, reviewed_overlay_fingerprint=self.reviewed_overlay_fingerprint, signature_version=self.signature_version)

    @classmethod
    def from_mention(cls, mention, registry, *, source_family=None, source_domain=None):
        return cls(project_id=mention.project_id, task_id=mention.task_id, platform=str(mention.platform), root_id=mention.root_id, language=mention.language, language_family=_language_family(mention.language), ontology_fingerprint=build_ontology_fingerprint(registry), overlay_fingerprint=build_overlay_fingerprint(registry), reviewed_overlay_fingerprint=build_reviewed_overlay_fingerprint(registry), signature_version=REVIEW_SIGNATURE_VERSION, source_family=source_family, source_domain=source_domain)
...
class SemanticSignature(BaseModel):
    ...
    def match_value(self, match_mode):
...
class ScopeKey(BaseModel):
    ...
    def _upgrade_legacy_payload(cls, value):
        if not isinstance(value, dict) or 'static_scope' in value or 'knowledge_state' in value:
            return value
        legacy = dict(value)
        return {'scope_level': legacy.get('scope_level'), 'static_scope': {'project_id': legacy.get('project_id'), 'task_id': legacy.get('task_id'), 'platform': legacy.get('platform'), 'root_id': legacy.get('root_id'), 'language_family': legacy.get('language_family'), 'signature_version': legacy.get('signature_version', REVIEW_SIGNATURE_VERSION)}, 'knowledge_state': {'ontology_fingerprint': legacy.get('ontology_fingerprint'), 'overlay_fingerprint': legacy.get('overlay_fingerprint'), 'reviewed_overlay_fingerprint': legacy.get('reviewed_overlay_fingerprint'), 'signature_version': legacy.get('signature_version', REVIEW_SIGNATURE_VERSION)}, 'signature_version': legacy.get('signature_version', REVIEW_SIGNATURE_VERSION)}
    ...
    @property
    def reviewed_overlay_fingerprint(self):
        return self.knowledge_state.reviewed_overlay_fingerprint

def build_review_context_index(mentions, registry, *, source_family=None, source_domain=None):
    return {mention.mention_id: ReviewContext.from_mention(mention, registry, source_family=source_family, source_domain=source_domain) for mention in mentions}
