...
class BatchProfile(BaseModel):
    ...
    def to_markdown(self):
        lines = ['# Batch Profile', '', f'- Total records: {self.total_records}', f'- Types: {self.records_by_type}', f'- Platforms: {self.records_by_platform}', '', '| Field | Present | Missing | Examples |', '| --- | ---: | ---: | --- |']
        for field_profile in self.field_profiles:
            lines.append('| {field} | {present} | {missing} | {examples} |'.format(field=field_profile.field_path, present=field_profile.present_count, missing=field_profile.missing_count, examples=', '.join(field_profile.example_values[:3]) or '-'))
        if self.notes:
            lines.extend(['', '## Notes', ''])
