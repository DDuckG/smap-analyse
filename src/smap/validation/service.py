...
def _present_count(field_stats: dict[str, FieldStats], *field_names: str) -> int:
    return max((field_stats.get(field_name, FieldStats()).present for field_name in field_names), default=0)


def validate_batch(path: Path | IngestedBatchBundle) -> ValidationReport:
...
def profile_batch(path: Path | IngestedBatchBundle) -> BatchProfile:
    if isinstance(path, IngestedBatchBundle):
    ...
    if total_records and _present_count(field_stats, "content.keywords", "content.tiktok_keywords") < total_records:
