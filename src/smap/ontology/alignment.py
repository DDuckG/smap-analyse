...
def _is_allowed(label: str, allowed: set[str]) -> bool:
    return label in allowed or label.startswith(PROVISIONAL_PREFIX)


def collect_alignment_errors(enrichment: EnrichmentBundle, ontology: OntologyRegistry) -> list[str]:
    errors: list[str] = []
    seen: set[str] = set()
