...
class PrototypeRegistry:
    ...
    def aspect_seed_map(self) -> dict[str, tuple[str, ...]]:
        return {bundle.aspect_id: bundle.seed_phrases for bundle in self.aspects.values()}

    def issue_seed_map(self) -> dict[str, tuple[str, ...]]:
        return {bundle.issue_id: bundle.seed_phrases for bundle in self.issues.values()}

    def eligible_target_keys(self) -> set[str]:
        return {
            bundle.canonical_entity_id
            for bundle in self.entities.values()
            if bundle.target_eligible
        }

    def eligible_concept_keys(self) -> set[str]:
        return {
            bundle.canonical_entity_id
            for bundle in self.entities.values()
            if bundle.entity_kind == "concept" and bundle.target_eligible
        }

    def topic_catalog_label(self, topic_key: str) -> str:
        bundle = self.topics.get(topic_key)
        return bundle.label if bundle is not None else topic_key.replace("_", " ").title()

    def _build_entities(self) -> dict[str, EntityPrototypeBundle]:
        bundles: dict[str, EntityPrototypeBundle] = {}
        for entity in self.ontology.entities:
            bundles[entity.id] = self._entity_bundle(entity)
        return bundles

    def _entity_bundle(self, entity: EntitySeed) -> EntityPrototypeBundle:
        ...
        related_topics = [
            topic for topic in self.ontology.topics if entity.id in topic.related_entity_ids
        ]
        neighboring_entity_ids = _unique_strings(
            [
                entity_id
            ...
            + entity.neighboring_entity_ids
        )
        neighboring_entity_labels = _unique_strings(
            [entity_labels[item] for item in neighboring_entity_ids if item in entity_labels]
        )
        neighboring_aspect_ids = _unique_strings(
            [
                aspect_id
                for topic in related_topics
                for aspect_id in topic.related_aspect_ids
            ]
            + entity.neighboring_aspect_ids
        )
        neighboring_aspect_labels = _unique_strings(
            [aspect_labels[item] for item in neighboring_aspect_ids if item in aspect_labels]
        )
        neighboring_issue_ids = _unique_strings(
            [
                issue_id
                for topic in related_topics
                for issue_id in topic.related_issue_ids
            ]
        )
        neighboring_issue_labels = _unique_strings(
            [issue_labels[item] for item in neighboring_issue_ids if item in issue_labels]
        )
        ...
        domain_anchor_phrases = _unique_strings(
            [
                *entity.domain_anchor_phrases,
                *[phrase for topic in related_topics for phrase in topic.seed_phrases[:4]],
                *related_topic_labels,
            ]
        )
        related_phrases = _unique_strings(
            [
                *entity.related_phrases,
                *taxonomy_labels,
                entity.entity_type,
                *neighboring_aspect_labels,
        ...
        return EntityPrototypeBundle(
            ...
            domain_anchor_phrases=domain_anchor_phrases,
            neighboring_entity_ids=neighboring_entity_ids,
            neighboring_entity_labels=neighboring_entity_labels,
            neighboring_aspect_ids=neighboring_aspect_ids,
            neighboring_aspect_labels=neighboring_aspect_labels,
            neighboring_issue_ids=neighboring_issue_ids,
            neighboring_issue_labels=neighboring_issue_labels,
            related_topic_keys=related_topic_keys,
            related_topic_labels=related_topic_labels,
            anti_confusion_phrases=anti_confusion,
            taxonomy_ids=tuple(entity.taxonomy_ids),
            prototype_text=" || ".join(_unique_strings(prototype_parts)),
            alias_lookup_text=" || ".join(_unique_strings(alias_lookup_parts)),
        )

    def _build_aspects(self) -> dict[str, AspectPrototypeBundle]:
        topic_labels = {topic.topic_key: topic.label for topic in self.ontology.topics}
            ...
            prototype_text = " || ".join(
                _unique_strings(
                    [
                        ...
                        *category.seed_phrases,
                        *related_issues,
                        *linked_topics,
                        *category.compatible_entity_types,
                    ]
                )
            )
            bundles[category.id] = AspectPrototypeBundle(
                aspect_id=category.id,
                label=category.label,
                seed_phrases=_unique_strings(category.seed_phrases),
                negative_phrases=_unique_strings(category.negative_phrases),
                compatible_entity_types=tuple(category.compatible_entity_types),
                linked_topic_keys=tuple(category.linked_topic_keys),
                related_issue_ids=tuple(category.related_issue_ids),
                prototype_text=prototype_text,
            )
        return bundles

    def _build_issues(self) -> dict[str, IssuePrototypeBundle]:
        topic_labels = {topic.topic_key: topic.label for topic in self.ontology.topics}
        ...
        for category in self.ontology.issue_categories:
            ...
            bundles[category.id] = IssuePrototypeBundle(
                issue_id=category.id,
                label=category.label,
                seed_phrases=_unique_strings(category.seed_phrases),
                negative_phrases=_unique_strings(category.negative_phrases),
                compatible_entity_types=tuple(category.compatible_entity_types),
                linked_topic_keys=tuple(category.linked_topic_keys),
                related_aspect_ids=tuple(category.related_aspect_ids),
                prototype_text=prototype_text,
            )
        return bundles

    def _build_topics(self) -> dict[str, TopicPrototypeBundle]:
        ...
        for topic in self.ontology.topics:
            ...
            bundles[topic.topic_key] = TopicPrototypeBundle(
                ...
                label=topic.label,
                knowledge_layer=topic.knowledge_layer,
                topic_family=topic.topic_family or topic.topic_key,
                main_path_policy=topic.main_path_policy,
                seed_phrases=_unique_strings(topic.seed_phrases),
                negative_phrases=_unique_strings(topic.negative_phrases),
                domain_tags=tuple(topic.domain_tags),
                excluded_domain_tags=tuple(topic.excluded_domain_tags),
                issue_centric=topic.issue_centric,
                reportable=topic.reportable,
                related_entity_ids=tuple(topic.related_entity_ids),
                related_aspect_ids=tuple(topic.related_aspect_ids),
                related_issue_ids=tuple(topic.related_issue_ids),
                compatible_entity_types=tuple(topic.compatible_entity_types),
                prototype_text=prototype_text,
            )
        return bundles
    ...
    def _precompute_vectors(self) -> None:
        ...
        entity_vectors = self.embedding_provider.embed_texts(
            [self.entities[entity_id].prototype_text for entity_id in entity_ids],
        ...
        topic_vectors = self.embedding_provider.embed_texts(
            [self.topics[topic_key].prototype_text for topic_key in topic_keys],
        ...
        aspect_vectors = self.embedding_provider.embed_texts(
            [self.aspects[aspect_id].prototype_text for aspect_id in aspect_ids],
            purpose=EmbeddingPurpose.LINKING,
        )
        ...
        issue_vectors = self.embedding_provider.embed_texts(
            [self.issues[issue_id].prototype_text for issue_id in issue_ids],
            purpose=EmbeddingPurpose.LINKING,
        )
...
def collect_phrase_lexicon(ontology: OntologyRegistry) -> tuple[str, ...]:
    phrases: list[str] = []
    ...
    scored = sorted(
        {
            normalize_alias(phrase)
            for phrase in phrases
            if len(normalize_alias(phrase).split()) >= 2 or any(character.isdigit() for character in normalize_alias(phrase))
        },
