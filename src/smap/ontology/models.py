...
CORE_SOURCE_CHANNEL_IDS = frozenset(platform.value for platform in Platform)
...
class OntologyRegistry(BaseModel):
    ...
    def validate_registry(self) -> OntologyRegistry:
        ...
        intent_ids = {category.id for category in self.intent_categories}
        issue_ids = {category.id for category in self.issue_categories}
        source_channel_ids = {channel.id for channel in self.source_channels}
        entity_ids = {entity.id for entity in self.entities}
        missing_shared_entity_types = sorted(SHARED_ENTITY_TYPE_IDS - set(self.entity_types))
        if missing_shared_entity_types:
            raise ValueError(
                f"ontology missing shared entity types {missing_shared_entity_types}"
            )
        missing_shared_intents = sorted(SHARED_INTENT_CATEGORY_IDS - intent_ids)
        if missing_shared_intents:
            raise ValueError(
                f"ontology missing shared intent categories {missing_shared_intents}"
            )
        missing_source_channels = sorted(CORE_SOURCE_CHANNEL_IDS - source_channel_ids)
        if missing_source_channels:
            raise ValueError(
                f"ontology missing core source channels {missing_source_channels}"
            )
        ...
        for entity in self.entities:
            ...
            if missing_taxonomy:
                raise ValueError(
                    f"entity {entity.id} references unknown taxonomy ids {missing_taxonomy}"
                )
            missing_neighbor_entities = [
                item for item in entity.neighboring_entity_ids if item not in entity_ids
            ]
            if missing_neighbor_entities:
                raise ValueError(
                    f"entity {entity.id} references unknown neighboring entities {missing_neighbor_entities}"
                )
            missing_neighbor_aspects = [
                item for item in entity.neighboring_aspect_ids if item not in aspect_ids
            ]
            if missing_neighbor_aspects:
                raise ValueError(
                    f"entity {entity.id} references unknown neighboring aspects {missing_neighbor_aspects}"
                )
        for topic in self.topics:
            missing_entity_ids = [
                item for item in topic.related_entity_ids if item not in entity_ids
            ]
            if missing_entity_ids:
                raise ValueError(
                    f"topic {topic.topic_key} references unknown entities {missing_entity_ids}"
                )
            missing_taxonomy = [item for item in topic.related_taxonomy_ids if item not in node_ids]
            if missing_taxonomy:
                raise ValueError(
                    f"topic {topic.topic_key} references unknown taxonomy ids {missing_taxonomy}"
                )
            missing_aspects = [item for item in topic.related_aspect_ids if item not in aspect_ids]
            if missing_aspects:
                raise ValueError(
                    f"topic {topic.topic_key} references unknown aspects {missing_aspects}"
                )
            missing_issues = [item for item in topic.related_issue_ids if item not in issue_ids]
            if missing_issues:
                raise ValueError(
                    f"topic {topic.topic_key} references unknown issues {missing_issues}"
                )
        ...
        for category in self.aspect_categories:
            missing_topics = [
                item
                for item in category.linked_topic_keys
                if item not in {topic.topic_key for topic in self.topics}
            ]
            if missing_topics:
        ...
        for category in self.issue_categories:
            ...
            missing_aspects = [
                item for item in category.related_aspect_ids if item not in aspect_ids
            ]
            if missing_aspects:
                raise ValueError(
                    f"issue {category.id} references unknown aspects {missing_aspects}"
                )
        return self
