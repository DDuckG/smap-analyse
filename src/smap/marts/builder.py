...
def build_marts(
    ...
    *,
    ...
    fact_entity_candidate_clusters = _frame_from_models(
    ...
    dim_projects = (
        fact_mentions.select(["project_id", "task_id", "platform"])
        .unique()
        .sort(["project_id", "task_id"])
        if not fact_mentions.is_empty()
        else pl.DataFrame([])
    )
    dim_time = (
        fact_mentions.with_columns(
            pl.col("posted_at").str.slice(0, 10).str.to_date("%Y-%m-%d", strict=False).alias("date")
        )
        .with_columns(
