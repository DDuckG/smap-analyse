...
def evaluate_manifest(manifest, expected):
    if manifest is None:
    ...
    if expected.embedding_provider_version is not None and expected.embedding_provider_version != manifest.embedding_provider_version:
...
def manifest_info(manifest, *, expected):
    if manifest is None and expected is None:
        return None
    if manifest is None and expected is not None:
        return VectorNamespaceInfo(namespace=expected.namespace, backend=expected.backend or 'unknown', item_count=0, dimension=expected.dimension or 0, normalization_mode=expected.normalization_mode or 'unknown', embedding_model_id=expected.embedding_model_id, embedding_provider_name=expected.embedding_provider_name, embedding_provider_version=expected.embedding_provider_version, expected_corpus_hash=expected.corpus_hash, reuse_state=VectorReuseState.MISSING.value, compatibility_errors=['manifest_missing'], recommended_action='vector-index-build')
