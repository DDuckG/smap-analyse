...
class OntologyGuidedTopicProvider(TopicProvider):
    def __init__(
        ...
        *,
        ...
        secondary_unmatched_topic_key: str = "topic.misc_unclassified",
    ) -> None:
        ...
        self.provenance = ProviderProvenance(
            provider_kind="topic",
            provider_name="ontology_topic",
            provider_version="ontology-topic-v3",
            model_id=(embedding_provider.provenance.model_id if embedding_provider is not None else "ontology-only"),
            device=(embedding_provider.provenance.device if embedding_provider is not None else "cpu"),
        )
        self._prototypes = [
            _TopicPrototype(
                definition=topic,
                prototype_text=" ".join(
                    part
                    for part in [topic.label, topic.description or "", *topic.seed_phrases]
                    if part
                ),
            )
        ...
        self._family_has_domain_primary = {
            (topic.topic_family or topic.topic_key)
            for topic in ontology.topics
    ...
    def discover(
        ...
        *,
        ...
        for index, document in enumerate(documents):
            topic_match = self._assign_document(
                document,
            ...
            assignments.append(
                TopicAssignment(
                    ...
                    topic_key=topic_definition.topic_key,
                    topic_label=topic_definition.label,
        ...
        for topic_key, topic_docs in sorted(docs_by_topic.items()):
            ...
            artifacts.append(
                TopicArtifact(
                    ...
                    representative_document_ids=tuple(document.document_id for document in topic_docs[:3]),
                    topic_size=len(topic_docs),
                    provider_provenance=self.provenance,
                    metadata={
                        "mode": "ontology_guided",
                        "knowledge_layer": topic_definition.knowledge_layer,
                    },
                )
            )
    ...
    def _assign_document(
        ...
        *,
        ...
        prototype_embeddings: dict[str, tuple[float, ...]],
    ) -> tuple[TopicDefinition, float, dict[str, ProviderMetadata]] | None:
        ...
        entity_ids = {
            str(item)
            for item in canonical_entity_ids
            if isinstance(item, str)
        } if isinstance(canonical_entity_ids, list) else set()
        aspect_metadata = document.metadata.get("aspect_ids")
        aspect_ids = {
            str(item)
            for item in aspect_metadata
            if isinstance(item, str)
        } if isinstance(aspect_metadata, list) else set()
        issue_metadata = document.metadata.get("issue_ids")
        issue_ids = {
            str(item)
            for item in issue_metadata
            if isinstance(item, str)
        } if isinstance(issue_metadata, list) else set()
        document_entity_types = {
            self._entity_type_by_id[item]
            for item in entity_ids
            if item in self._entity_type_by_id
        }
        ...
        for prototype in self._prototypes:
            ...
            domain_profile_eligible = self._topic_matches_active_domain(
                prototype.definition,
                document_entity_types=document_entity_types,
                domain_grounded=domain_grounded,
                lexical=lexical,
                entity_bonus=entity_bonus,
                aspect_bonus=aspect_bonus,
                issue_bonus=issue_bonus,
            )
            domain_primary_bonus = (
                0.1
                if prototype.definition.main_path_policy == "domain_primary"
                and (lexical >= 0.2 or entity_bonus > 0.0 or aspect_bonus > 0.0 or issue_bonus > 0.0)
                else 0.0
            )
            family_penalty = 0.0
            ...
            issue_leak_penalty = (
                0.14
                if issue_bonus > 0.0
                and not prototype.definition.issue_centric
                and aspect_bonus == 0.0
                and entity_bonus == 0.0
                and lexical < 0.42
                else 0.0
            )
            compatibility_bonus = (
                0.06
                if document_entity_types
                and prototype.definition.compatible_entity_types
                and document_entity_types & set(prototype.definition.compatible_entity_types)
                else 0.0
            )
            if not self._topic_is_main_path_eligible(
                prototype.definition,
                domain_grounded=domain_grounded,
                lexical=lexical,
                entity_bonus=entity_bonus,
                ...
                issue_bonus=issue_bonus,
                document_entity_types=document_entity_types,
                domain_profile_eligible=domain_profile_eligible,
            ):
                continue
            score = (
                lexical * 0.54
            ...
            if score >= 0.32:
                scored.append(
                    (
                        ...
                        min(score, 0.98),
                        {
                            "assignment_mode": "ontology_guided",
                            "lexical_score": round(lexical, 4),
                            "semantic_score": round(semantic, 4),
                            "entity_bonus": round(entity_bonus, 4),
                            "aspect_bonus": round(aspect_bonus, 4),
                            "issue_bonus": round(issue_bonus, 4),
                            "domain_primary_bonus": round(domain_primary_bonus, 4),
                            "compatibility_bonus": round(compatibility_bonus, 4),
                            "family_penalty": round(family_penalty, 4),
                            "issue_leak_penalty": round(issue_leak_penalty, 4),
                            "domain_profile_eligible": domain_profile_eligible,
    ...
    def _lexical_score(self, normalized_text: str, topic: TopicDefinition) -> tuple[float, list[str]]:
    ...
    def _semantic_score(
        ...
        *,
        ...
        query_embedding = (
            ...
            else self.embedding_provider.embed_texts([normalized_text], purpose=EmbeddingPurpose.CLUSTERING)[0]
        )
        return max(sum(a * b for a, b in zip(query_embedding, topic_embedding, strict=True)), 0.0)

    def _prototype_embeddings(self) -> dict[str, tuple[float, ...]]:
        ...
        vectors = self.embedding_provider.embed_texts(
            [prototype.prototype_text for prototype in self._prototypes],
            purpose=EmbeddingPurpose.CLUSTERING,
        )
        self._prototype_embedding_cache = {
            prototype.definition.topic_key: vector
            for prototype, vector in zip(self._prototypes, vectors, strict=True)
        }
        return self._prototype_embedding_cache

    def _top_terms(self, documents: list[TopicDocument], topic: TopicDefinition) -> list[str]:
    ...
    def _topic_matches_active_domain(
        self,
        topic: TopicDefinition,
        *,
        document_entity_types: set[str],
    ...
    def _topic_is_main_path_eligible(
        self,
        topic: TopicDefinition,
        *,
        domain_grounded: bool,
        ...
        compatible_type_match = bool(
            document_entity_types
            and topic.compatible_entity_types
            and (document_entity_types & set(topic.compatible_entity_types))
        )
        if self._topic_family_is_domain_owned(topic) and topic.main_path_policy != "domain_primary":
            return domain_profile_eligible and (
                lexical >= 0.74
                and (
                    entity_bonus > 0.0
                    or aspect_bonus > 0.0
                    or issue_bonus > 0.0
                    or (compatible_type_match and lexical >= 0.82)
                )
            )
        return domain_profile_eligible and (
            lexical >= 0.58
            ...
            or aspect_bonus > 0.0
            or issue_bonus > 0.0
            or compatible_type_match
        )

    def _topic_family_is_domain_owned(self, topic: TopicDefinition) -> bool:
        return (topic.topic_family or topic.topic_key) in self._family_has_domain_primary


def semantic_bonus_gate(
    *,
    topic: TopicDefinition,
    lexical: float,
    entity_bonus: float,
