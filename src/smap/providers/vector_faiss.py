...
from smap.providers.vector_index_manifest import (
    build_manifest,
    load_manifest,
    namespace_dir,
    write_manifest,
...
class FaissVectorIndex(VectorIndex):
    def __init__(self, *, storage_dir: Path | None = None) -> None:
        ...
        self.provenance = ProviderProvenance(
            provider_kind="vector_index",
            provider_name="faiss",
            provider_version=self.version,
            model_id="IndexFlatIP",
            device="cpu",
        )
    ...
    def upsert(self, items: list[VectorItem], *, namespace: str) -> None:
        ...
        if self.storage_dir is not None:
            ...
            manifest = build_manifest(
                ...
                previous=load_manifest(self.storage_dir, namespace),
            )
    ...
    def info(
        ...
        *,
        ...
        return VectorNamespaceInfo(
            ...
            embedding_model_id=str(first_metadata.get("embedding_model_id"))
            if first_metadata.get("embedding_model_id") is not None
            else None,
            embedding_provider_name=str(first_metadata.get("embedding_provider_name"))
            if first_metadata.get("embedding_provider_name") is not None
            else None,
            embedding_provider_version=str(first_metadata.get("embedding_provider_version"))
            if first_metadata.get("embedding_provider_version") is not None
            else None,
            storage_path=str(namespace_dir(self.storage_dir, namespace)) if self.storage_dir is not None else None,
            expected_corpus_hash=expected.corpus_hash if expected is not None else None,
        )
    ...
    def search(
        ...
        *,
        ...
        for score, index in zip(scores[0], indices[0], strict=True):
            ...
            hits.append(
                VectorSearchHit(
                    item_id=state.item_ids[int(index)],
                    score=round(float(score), 6),
                    text=state.texts[int(index)],
                    metadata=state.metadata[int(index)],
                )
            )
        return hits
