...
_GENERIC_TAXONOMY_PROTOTYPES: dict[str, tuple[str, ...]] = {
    "usability": ("user flow", "setup flow", "navigation", "interface", "checkout flow", "de su dung", "luong thao tac"),
    "usability_problem": ("user flow issue", "setup issue", "navigation problem", "interface problem", "checkout friction", "kho dung", "roi flow"),
    "customer_service": (
        "support quality",
        "response quality",
        "staff support",
    ...
    "performance": ("speed", "loading", "stability", "lag", "do tre"),
    "performance_problem": ("slow loading", "performance issue", "stability issue", "bi lag", "qua cham", "tre"),
    "delivery": ("shipping speed", "delivery timing", "giao hang"),
    "delivery_problem": ("shipping delay", "delivery delay", "ship cham", "giao tre"),
    "pricing_value_concern": ("too expensive", "poor value", "price concern"),
    "trust_concern": ("authenticity", "credibility concern", "trust issue", "khong tin", "fake"),
}
...
class EmbeddingTaxonomyMappingProvider(TaxonomyMappingProvider):
    ...
    def __post_init__(self) -> None:
        self.provenance = ProviderProvenance(
            provider_kind="semantic_assist",
            provider_name="embedding_taxonomy_mapping",
            provider_version=self.version,
            model_id=self.embedding_provider.provenance.model_id,
            device=self.embedding_provider.provenance.device,
        )
    ...
    def map_candidates(
        ...
        *,
        ...
        if prototype_texts is None or prototype_vectors is None:
            ...
            prototype_embeddings = self.embedding_provider.embed_texts(
                [prototype_texts[prototype_id][1] for prototype_id in prototype_ids],
        ...
        label_vectors = dict(
            zip(
                unique_labels,
                self.embedding_provider.embed_texts(unique_labels, purpose=EmbeddingPurpose.LINKING),
                strict=True,
            )
        )
        ...
        for label in unique_labels:
            ...
            if query_vector is not None:
                ranked = sorted(
                    [
                        SimilarityMatch(
                            candidate_id=prototype_id,
                            score=round(
                                sum(a * b for a, b in zip(query_vector, prototype_vectors[prototype_id], strict=True)),
                                6,
                            ),
                            candidate_text=prototype_texts[prototype_id][1],
                        )
                        for prototype_id in prototype_texts
                    ],
            ...
            mapped[label] = sorted(
                by_taxonomy.values(),
                key=lambda item: (-item.score, item.taxonomy_label),
            )[:top_k]
...
class EmbeddingRerankerProvider(RerankerProvider):
    ...
    def __post_init__(self) -> None:
        self.provenance = ProviderProvenance(
            provider_kind="semantic_assist",
            provider_name="embedding_reranker",
            provider_version=self.version,
            model_id=self.embedding_provider.provenance.model_id,
            device=self.embedding_provider.provenance.device,
        )
    ...
    def rerank(
        ...
        return [
            SimilarityMatch(
                candidate_id=candidate_map.get(match.candidate_id, match.candidate_id),
                score=match.score,
                candidate_text=candidate_map.get(match.candidate_id, match.candidate_id),
...
def _taxonomy_prototypes(taxonomy_label: str) -> tuple[str, ...]:
...
def _prototype_variants(text: str) -> tuple[str, ...]:
...
def _normalized_variant(text: str) -> str:
