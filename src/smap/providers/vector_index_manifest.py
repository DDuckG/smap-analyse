...
class VectorNamespaceManifest(BaseModel):
    ...
    def as_info(self) -> VectorNamespaceInfo:
...
def manifest_path(root: Path, namespace: str) -> Path:
...
def build_manifest(
    *,
    ...
    storage_path: Path,
    previous: VectorNamespaceManifest | None = None,
) -> VectorNamespaceManifest:
    ...
    return VectorNamespaceManifest(
        ...
        embedding_model_id=_string_metadata(first_metadata, "embedding_model_id"),
        embedding_provider_name=_string_metadata(first_metadata, "embedding_provider_name"),
        embedding_provider_version=_string_metadata(first_metadata, "embedding_provider_version"),
        ...
        metadata={
            "namespace": namespace,
            "embedding_purpose": _string_metadata(first_metadata, "embedding_purpose"),
        },
    )


def load_manifest(root: Path, namespace: str) -> VectorNamespaceManifest | None:
...
def write_manifest(root: Path, manifest: VectorNamespaceManifest) -> None:
    path = manifest_path(root, manifest.namespace)
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(manifest.model_dump_json(indent=2), encoding="utf-8")


def corpus_hash(items: list[VectorItem]) -> str:
    payload = [
        {
...
def _string_metadata(metadata: dict[str, ProviderMetadata], key: str) -> str | None:
