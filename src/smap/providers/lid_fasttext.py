...
from smap.providers.lid_heuristic import best_language_candidate, canonicalize_language_label, heuristic_language_candidates

class FastTextLanguageIdProvider(LanguageIdProvider):

    def __init__(self, *, model_path, mixed_confidence_threshold=0.55, mixed_gap_threshold=0.12, script_override_enabled=True):
        try:
            import fasttext
        ...
        self.provenance = ProviderProvenance(provider_kind='language_id', provider_name='fasttext_lid', provider_version=package_version, model_id=model_path.name, device='cpu', run_metadata={'model_path': str(model_path), 'mixed_confidence_threshold': mixed_confidence_threshold, 'mixed_gap_threshold': mixed_gap_threshold, 'script_override_enabled': script_override_enabled})
        self._model: Any | None = None

    def detect(self, text):
        if not text.strip():
            return LanguageIdentificationResult(language='unknown', confidence=0.0, provider_provenance=self.provenance, metadata={'reason': 'empty_text'})
        model = self._model_instance()
        labels, probabilities = self._predict_topk(model, text.replace('\n', ' '), k=2)
        candidates = {canonicalize_language_label(label): round(float(probability), 4) for label, probability in zip(labels, probabilities, strict=True)}
        ...
        return LanguageIdentificationResult(language=language, confidence=confidence, provider_provenance=self.provenance, metadata={'top_candidates': ', '.join((f'{item[0]}:{item[1]:.3f}' for item in top_candidates))})
    ...
    def _model_instance(self):
        if self._model is None:
            try:
                ...
                raise ProviderUnavailableError(f'Unable to load fastText language ID model from `{self.model_path}`.') from exc
        return self._model

    def _predict_topk(self, model, text, *, k):
