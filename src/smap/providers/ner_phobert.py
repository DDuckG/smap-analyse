...
_MODEL_CODE_RE = re.compile(r"\b[a-z]{1,10}[- ]?\d{1,4}[a-z0-9-]{0,4}\b", re.IGNORECASE)
_ENTITY_TOKEN_RE = re.compile(r"[A-Za-z0-9][A-Za-z0-9-]{1,}")
_CAMEL_TITLE_RE = re.compile(r"\b[A-ZÀ-Ỹ][\w-]+(?:\s+[A-ZÀ-Ỹ][\w-]+){0,3}\b")
_GENERIC_SURFACES = {
    "brand",
    "company",
    "customer service",
...
_TYPE_PROTOTYPES = {
    "brand": "thuong hieu brand hang xe cong ty brand name",
    "product": "san pham product model dong xe phien ban variant mau xe",
    "organization": "to chuc organization tap doan company doanh nghiep",
    "person": "nhan vat person founder chairman ceo nguoi noi tieng",
    "location": "dia diem location thanh pho quoc gia thi truong market",
    "facility": "nha may factory xưởng plant facility production site",
    "retailer": "dai ly showroom dealer retailer cua hang store",
    "concept": "khai niem concept chu de generic",
}
...
class PhoBERTNERProvider(NERProvider):
    def __init__(
        ...
        *,
        ...
        self.provenance = ProviderProvenance(
            provider_kind="ner",
            provider_name="phobert_ner",
            provider_version=package_version,
            model_id=model_id,
            device=embedding_provider.provenance.device,
            run_metadata={"min_similarity": min_similarity},
        )
        ...
        for start, end, surface in self._span_proposals(text):
            ...
            if top_alias is not None and top_alias[2] >= max(self.min_similarity, 0.66):
                ...
                spans.append(
                    RecognizedEntitySpan(
                        ...
                        confidence=round(min(max(top_alias[2], 0.72), 0.99), 3),
                        provider_provenance=self.provenance,
                        metadata={"source": "phobert_ner", "matched_entity_id": top_alias[0]},
                    )
                )
                continue
            if top_type is None:
            ...
            spans.append(
                RecognizedEntitySpan(
                    ...
                    confidence=round(min(final_score, 0.92), 3),
                    provider_provenance=self.provenance,
                    metadata={"source": "phobert_ner"},
                )
            )
        return sorted(spans, key=lambda item: (item.start, item.end, item.label))

    def _span_is_worthy(self, surface: str, normalized: str, entity_type: str) -> bool:
        ...
        if entity_type in {"brand", "organization", "retailer"}:
            return len(compact) >= 4 and (
                ...
                or len(tokens) >= 2
                or compact in self._alias_token_index
            )
    ...
    def _span_proposals(self, text: str) -> list[tuple[int, int, str]]:
    ...
    def _supports_title_surface(self, surface: str, normalized: str) -> bool:
    ...
    def _supports_combined_surface(self, surface: str) -> bool:
        normalized = normalize_alias(surface)
        if normalized in _GENERIC_SURFACES:
            return False
        tokens = [token for token in normalized.split() if token]
    ...
    def _proposal_is_business_worthy(self, surface: str, normalized: str, *, exact_alias: bool) -> bool:
        if normalized in _GENERIC_SURFACES:
            return False
        if any(character.isdigit() for character in normalized):
            return True
    ...
    def _covered_by_longer_proposal(self, start: int, end: int, proposals: list[tuple[int, int, str]]) -> bool:
    ...
    def _top_alias_match(self, surface: str, normalized: str) -> tuple[str, str, float] | None:
        ...
        ranked = self.embedding_provider.rank_candidates(
            normalized,
            {alias.alias_id: alias.alias for alias in shortlist},
    ...
    def _top_type_match(
        ...
        type_candidates = {
            ...
            for entity_type, prototype in _TYPE_PROTOTYPES.items()
    ...
    def _shortlist_aliases(self, normalized: str) -> list[EntityAlias]:
        tokens = {
            token
        ...
        alias_ids = {
            alias_id
            for token in tokens
            for alias_id in self._alias_token_index.get(token, set())
        }
        if not alias_ids and any(character.isdigit() for character in normalized):
            alias_ids = {
                alias.alias_id
                for alias in self.alias_registry.all_aliases()
                if any(character.isdigit() for character in alias.normalized_alias)
            }
        shortlist = [
            alias
            for alias in self.alias_registry.all_aliases()
    ...
    def _build_alias_token_index(self) -> dict[str, set[str]]:
    ...
    def _build_entity_types(self) -> list[str]:
        return sorted(
            {
                entity.entity_type
                for entity in self.alias_registry.entities.values()
                if entity.entity_type in _TYPE_PROTOTYPES
            }
        )
