...
class EntityExtractionEnricher:
    ...
    def __init__(
        ...
        discoverer: EntityCandidateDiscoverer | None = None,
        ontology_registry: OntologyRegistry | None = None,
        other_domain_aliases: frozenset[str] | None = None,
        ner_provider_builder: Callable[[AliasRegistry], list[NERProvider]] | None = None,
    ...
    def _scope_fingerprint(self, context: ReviewContext) -> str:
        return hashlib.sha256(
            json.dumps(context.model_dump(mode="json"), sort_keys=True, separators=(",", ":")).encode("utf-8")
        ).hexdigest()[:16]
    ...
    def prepare(
        ...
        for scope_fingerprint, scoped_mentions in mentions_by_scope.items():
            ...
            alias_registry = AliasRegistry.from_ontology(
                self.ontology_registry,
                review_context=review_context,
            )
            engine = CanonicalizationEngine(
                alias_registry=alias_registry,
                embedding_provider=self.engine.embedding_provider,
                vector_index=self.engine.vector_index,
                fuzzy_threshold=self.engine.fuzzy_threshold,
                embedding_threshold=self.engine.embedding_threshold,
                vector_namespace=f"canonical:{scope_fingerprint}",
            )
    ...
    def discover_candidates_for_mention(
        self,
        ...
        return self.candidate_merger.merge(
            base_candidates=base_candidates,
            recognized_spans=recognized_spans,
            mention_id=mention.mention_id,
            source_uap_id=mention.source_uap_id,
            context_text=context.context_text if context else None,
            surrounding_text=mention.raw_text,
        )

    def enrich(self, mention: MentionRecord, context: MentionContext | None) -> list[EntityFact]:
        scope_fingerprint = self._scope_by_mention.get(mention.mention_id)
        ...
        for candidate in candidates:
            ...
            surface_specificity = engine._surface_specificity(
                candidate,
                candidate.normalized_text or candidate.text,
            )
            fact = EntityFact(
                mention_id=mention.mention_id,
                source_uap_id=mention.source_uap_id,
                ...
                concept_entity_id=decision.concept_entity_id,
                entity_type=entity_type,
                confidence=max(decision.confidence, candidate.confidence if decision.canonical_entity_id else 0.0),
                matched_by=decision.matched_by,
                resolution_kind=decision.resolution_kind,
                resolved_entity_kind=decision.resolved_entity_kind,
                knowledge_layer=decision.knowledge_layer,
                target_eligible=resolved_entity.target_eligible if resolved_entity is not None else False,
                surface_specificity=surface_specificity,
                unresolved_reason=(
                    None
                    if decision.canonical_entity_id is not None or decision.concept_entity_id is not None
                    else self._normalize_unresolved_reason(
                        mention,
                        candidate,
                        decision.unresolved_reason,
                    )
                ),
                canonical_candidate_ids=decision.candidate_canonical_ids,
                discovered_by=candidate.discovered_by,
                provenance=FactProvenance(
                    source_uap_id=mention.source_uap_id,
                    mention_id=mention.mention_id,
                    provider_version=decision.provider_version,
                    rule_version="entity-discovery-resolution-v4",
                    evidence_text=candidate.surrounding_text or mention.raw_text,
            ...
            if fact.canonical_entity_id is not None or fact.concept_entity_id is not None:
                if not self._keep_resolved_fact(fact):
                    unresolved_facts.append(
                        (
                            round(candidate.confidence + (surface_specificity * 0.18), 4),
                            fact.model_copy(
                                update={
                                    ...
                                    "unresolved_reason": "rejected_by_business_lane_eligibility",
                                }
                            ),
                        )
                    )
        ...
        if resolved_facts:
            return sorted(
                resolved_facts.values(),
                key=lambda item: (-item.confidence, item.entity_type or "", item.candidate_text),
            )
        if unresolved_facts:
    ...
    def _normalize_unresolved_reason(
        self,
        mention: MentionRecord,
    ...
    def _no_candidate_reason(self, mention: MentionRecord) -> str:
    ...
    def _matches_other_domain_catalog(self, text: str) -> bool:
    ...
    def _unresolved_fact(self, mention: MentionRecord, *, reason: str) -> EntityFact:
        return EntityFact(
            mention_id=mention.mention_id,
            source_uap_id=mention.source_uap_id,
            candidate_text=mention.normalized_text_compact or mention.raw_text,
            entity_type=None,
            confidence=0.0,
            matched_by="unresolved",
            resolution_kind="unresolved_candidate",
            ...
            provenance=FactProvenance(
                source_uap_id=mention.source_uap_id,
                mention_id=mention.mention_id,
                provider_version=self.engine.embedding_provider.version,
                rule_version="entity-discovery-resolution-v4",
                evidence_text=mention.raw_text,
            ),
        )

    def _keep_resolved_fact(self, fact: EntityFact) -> bool:
