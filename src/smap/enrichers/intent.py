...
class IntentEnricher:
    ...
    def enrich(self, mention, context):
        ...
        if '?' in mention.raw_text or 'hỏi' in text:
            ...
            intent = 'commentary'
            confidence = 0.4
        return [IntentFact(mention_id=mention.mention_id, source_uap_id=mention.source_uap_id, intent=intent, confidence=confidence, provenance=FactProvenance(source_uap_id=mention.source_uap_id, mention_id=mention.mention_id, provider_version='intent-rules-v1', rule_version='intent-rules-v1', evidence_text=context.context_text if context else mention.raw_text))]
